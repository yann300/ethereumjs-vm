import { EthereumJSErrorWithoutCode, Lock, bytesToHex, equalsBytes, intToHex, matchingBytesLength, } from '@ethereumjs/util';
...
import { LeafVerkleNodeValue } from "./node/types.js";
...
export class VerkleTree {
    ...
    async get(stem, suffixes) {
        ...
        if (res.node instanceof LeafVerkleNode) {
            ...
            for (const suffix of suffixes) {
                ...
                this.DEBUG &&
                    this.debug(`Suffix: ${suffix}; Value: ${value === undefined ? 'undefined' : bytesToHex(value)}`, ['get']);
                values.push(value);
            }
            return values;
        }
    ...
    async put(stem, suffixes, values = []) {
        ...
        if (values.length > 0 && values.length !== suffixes.length) {
            ...
            throw EthereumJSErrorWithoutCode(`expected number of values; ${values.length} to equal ${suffixes.length}`);
        }
        ...
        if (foundPath.node !== null) {
            ...
            if (!isLeafVerkleNode(foundPath.node)) {
                throw EthereumJSErrorWithoutCode(`expected leaf node found at ${bytesToHex(stem)}. Got internal node instead`);
            }
            leafNode = foundPath.node;
            // Sanity check to verify we have the right leaf node
            if (!equalsBytes(leafNode.stem, stem)) {
                throw EthereumJSErrorWithoutCode(`invalid leaf node found. Expected stem: ${bytesToHex(stem)}; got ${bytesToHex(foundPath.node.stem)}`);
            }
        }
        else {
        ...
        for (let i = 0; i < values.length; i++) {
            ...
            this.DEBUG &&
                this.debug(`Updating value for suffix: ${suffix} to value: ${value instanceof Uint8Array ? bytesToHex(value) : value} at leaf node with stem: ${bytesToHex(stem)}`, ['put']);
        }
        ...
        if (foundPath.stack.length > 1) {
            ...
            while (foundPath.stack.length > 1) {
                ...
                if (childReference !== null) {
                    ...
                    this.DEBUG &&
                        this.debug(`Updating child reference for node with path: ${bytesToHex(lastPath)} at index ${childIndex} in internal node at path ${bytesToHex(nextPath)}`, ['put']);
                    ...
                    this.DEBUG &&
                        this.debug(`Deleting child reference for node with path: ${bytesToHex(lastPath)} at index ${childIndex} in internal node at path ${bytesToHex(nextPath)}`, ['put']);
                }
        ...
        this.DEBUG &&
            this.debug(`Updating child reference for node with path: ${bytesToHex(lastPath)} at index ${lastPath[0]} in root node`, ['put']);
    ...
    async updateParent(leafNode, nearestNode, pathToNode) {
        const leafNodeWasDeleted = leafNode.values.filter((val) => val !== LeafVerkleNodeValue.Untouched).length === 0;
            ...
            if (leafNodeWasDeleted) {
                ...
                if (children.length === 1) {
                    ...
                    if (rawNode === undefined)
                        throw EthereumJSErrorWithoutCode(`missing node in DB at ${bytesToHex(children[0].path)}`);
                    return {
                        node: decodeVerkleNode(rawNode, this.verkleCrypto),
                        lastPath: children[0].path,
                    };
                }
            }
            this.DEBUG &&
                this.debug(`Updating child reference for leaf node with stem: ${bytesToHex(leafNode.stem)} at index ${leafNode.stem[partialMatchingStemIndex]} in internal node at path ${bytesToHex(leafNode.stem.slice(0, partialMatchingStemIndex))}`, ['put']);
        }
        else {
    ...
    async findPath(key) {
        ...
        if (rawNode === undefined)
            throw EthereumJSErrorWithoutCode('root node should exist');
        const rootNode = decodeVerkleNode(rawNode, this.verkleCrypto);
        ...
        while (!finished) {
            ...
            if (foundNode || child.path.length >= key.length || isLeafVerkleNode(decodedNode)) {
                ...
                if (foundNode) {
                    this.DEBUG &&
                        this.debug(`Path ${bytesToHex(key)} - found full path to node ${bytesToHex(decodedNode.hash())}.`, ['find_path']);
                ...
                this.DEBUG &&
                    this.debug(`Path ${bytesToHex(pathToNearestNode)} - found path to nearest node ${bytesToHex(decodedNode.hash())} but target node not found.`, ['find_path']);
            ...
            this.DEBUG &&
                this.debug(`Partial Path ${bytesToHex(key.slice(0, matchingKeyLength))} - found next node in path ${bytesToHex(decodedNode.hash())}.`, ['find_path']);
