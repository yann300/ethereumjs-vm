...
const checkpoint_ts_1 = require("./db/checkpoint.js");
const internalNode_ts_1 = require("./node/internalNode.js");
const leafNode_ts_1 = require("./node/leafNode.js");
const types_ts_1 = require("./node/types.js");
const util_ts_1 = require("./node/util.js");
const types_ts_2 = require("./types.js");
/**
...
class VerkleTree {
    ...
    constructor(opts) {
        ...
        this._opts = opts;
        if (opts.db instanceof checkpoint_ts_1.CheckpointDB) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Cannot pass in an instance of CheckpointDB');
        }
        this._db = new checkpoint_ts_1.CheckpointDB({ db: opts.db, cacheSize: opts.cacheSize });
    ...
    async get(stem, suffixes) {
        ...
        if (res.node instanceof leafNode_ts_1.LeafVerkleNode) {
    ...
    async put(stem, suffixes, values = []) {
        ...
        if (values.length > 0 && values.length !== suffixes.length) {
            ...
            throw (0, util_1.EthereumJSErrorWithoutCode)(`expected number of values; ${values.length} to equal ${suffixes.length}`);
        }
        ...
        if (foundPath.node !== null) {
            ...
            if (!(0, util_ts_1.isLeafVerkleNode)(foundPath.node)) {
                throw (0, util_1.EthereumJSErrorWithoutCode)(`expected leaf node found at ${(0, util_1.bytesToHex)(stem)}. Got internal node instead`);
            }
            leafNode = foundPath.node;
            // Sanity check to verify we have the right leaf node
            if (!(0, util_1.equalsBytes)(leafNode.stem, stem)) {
                throw (0, util_1.EthereumJSErrorWithoutCode)(`invalid leaf node found. Expected stem: ${(0, util_1.bytesToHex)(stem)}; got ${(0, util_1.bytesToHex)(foundPath.node.stem)}`);
            }
        }
        else {
            // Leaf node doesn't exist, create a new one
            leafNode = await leafNode_ts_1.LeafVerkleNode.create(stem, this.verkleCrypto);
        ...
        for (let i = 0; i < values.length; i++) {
            ...
            if (value !== types_ts_1.LeafVerkleNodeValue.Untouched && (0, util_1.equalsBytes)(value, (0, util_ts_1.createZeroesLeafValue)())) {
                // Special case for when the deleted leaf value or zeroes is passed to `put`
                // Writing the deleted leaf value to the suffix
                leafNode.setValue(suffix, types_ts_1.LeafVerkleNodeValue.Deleted);
            }
            else {
        ...
        if (leafNode.values.every((val) => val === types_ts_1.LeafVerkleNodeValue.Untouched)) {
            ...
            if (foundPath.node !== null) {
                ...
                this.DEBUG && this.debug(`Deleting leaf node at stem: ${(0, util_1.bytesToHex)(stem)}`, ['put']);
        ...
        if (foundPath.stack.length > 1) {
            ...
            while (foundPath.stack.length > 1) {
                ...
                if (childReference !== null) {
                    ...
                    if ((0, util_1.equalsBytes)(nextNode.commitment, this.verkleCrypto.zeroCommitment)) {
                    ...
                    this.DEBUG &&
                        this.debug(`Deleting child reference for node with path: ${(0, util_1.bytesToHex)(lastPath)} at index ${childIndex} in internal node at path ${(0, util_1.bytesToHex)(nextPath)}`, ['put']);
                }
    ...
    async del(stem, suffixes) {
        ...
        await this.put(stem, suffixes, new Array(suffixes.length).fill((0, util_ts_1.createZeroesLeafValue)()));
    }
    /**
     ...
     */
    async updateParent(leafNode, nearestNode, pathToNode) {
        const leafNodeWasDeleted = leafNode.values.filter((val) => val !== types_ts_1.LeafVerkleNodeValue.Untouched).length === 0;
        ...
        let internalNode;
        if ((0, util_ts_1.isLeafVerkleNode)(nearestNode) && !leafNodeWasDeleted) {
            // We need to create a new internal node and set nearestNode and leafNode as child nodes of it
            // Create new internal node
            internalNode = internalNode_ts_1.InternalVerkleNode.create(this.verkleCrypto);
        ...
        else if (!(0, util_ts_1.isLeafVerkleNode)(nearestNode)) {
            ...
            internalNode.setChild(leafNode.stem[partialMatchingStemIndex], leafNodeWasDeleted
                ? null
                : {
                    commitment: leafNode.commitment,
                    path: leafNode.stem,
            ...
            if (leafNodeWasDeleted) {
                ...
                if (children.length === 1) {
                    ...
                    const rawNode = await this._db.get(this.verkleCrypto.hashCommitment(children[0].commitment));
                    if (rawNode === undefined)
                        throw (0, util_1.EthereumJSErrorWithoutCode)(`missing node in DB at ${(0, util_1.bytesToHex)(children[0].path)}`);
                    return {
                        node: (0, util_ts_1.decodeVerkleNode)(rawNode, this.verkleCrypto),
    ...
    async findPath(key) {
        ...
        const rootNode = (0, util_ts_1.decodeVerkleNode)(rawNode, this.verkleCrypto);
        ...
        while (!finished) {
            ...
            if (rawNode === undefined)
                throw (0, util_1.EthereumJSErrorWithoutCode)(`missing node at ${(0, util_1.bytesToHex)(child.path)}`);
            const decodedNode = (0, util_ts_1.decodeVerkleNode)(rawNode, this.verkleCrypto);
            ...
            if (foundNode || child.path.length >= key.length || (0, util_ts_1.isLeafVerkleNode)(decodedNode)) {
                ...
                const pathToNearestNode = (0, util_ts_1.isLeafVerkleNode)(decodedNode) ? decodedNode.stem : child.path;
                this.DEBUG &&
    ...
    async createRootNode() {
        const rootNode = new internalNode_ts_1.InternalVerkleNode({
    ...
    async persistRoot() {
        if (this._opts.useRootPersistence === true) {
            await this._db.put(types_ts_2.ROOT_DB_KEY, this.root());
        }
    }
    /**
