...
function pedersenHash(input) {
    ...
    if (pedersenHash === null) {
        throw new Error(`pedersenHash: Wrong pedersenHash input: ${(0, util_1.bytesToHex)(input)}. This might happen if length is not correct.`);
    }
    return pedersenHash;
}
...
function getStem(address, treeIndex = 0) {
    const address32 = (0, util_1.setLengthLeft)(address.toBytes(), 32);
    let treeIndexBytes;
    if (typeof treeIndex === 'number') {
        treeIndexBytes = (0, util_1.setLengthRight)((0, util_1.int32ToBytes)(Number(treeIndex), true), 32);
    }
    else {
        treeIndexBytes = (0, util_1.setLengthRight)((0, util_1.bigIntToBytes)(BigInt(treeIndex), true).slice(0, 32), 32);
    }
    const input = (0, util_1.concatBytes)(address32, treeIndexBytes);
...
function getKey(stem, subIndex) {
    const treeKey = (0, util_1.concatBytes)(stem, subIndex);
    return treeKey;
}
