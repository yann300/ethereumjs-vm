...
exports.StatefulBinaryTreeStateManager = void 0;
const binarytree_1 = require("@ethereumjs/binarytree");
...
class StatefulBinaryTreeStateManager {
    constructor(opts) {
        ...
        this.getAccount = async (address) => {
            ...
            const stem = (0, util_1.getBinaryTreeStem)(this.hashFunction, address, 0);
            // First retrieve the account "header" values from the trie
            const accountValues = await this._tree.get(stem, [
                util_1.BinaryTreeLeafType.BasicData,
                util_1.BinaryTreeLeafType.CodeHash,
            ]);
            let account;
            if (accountValues[0] !== null && accountValues[0] !== undefined) {
                const basicData = (0, util_1.decodeBinaryTreeLeafBasicData)(accountValues[0]);
                account = (0, util_1.createPartialAccount)({
                    ...
                    codeHash: accountValues[1] === null ||
                        ...
                        (0, util_1.equalsBytes)(accountValues[1], new Uint8Array(32))
                        ? util_1.KECCAK256_NULL
                        : accountValues[1],
                    codeSize: basicData.codeSize,
                    storageRoot: util_1.KECCAK256_NULL,
                });
            }
        ...
        this.putAccount = async (address, account) => {
            ...
            if (this._caches?.account === undefined) {
                if (account !== undefined) {
                    const stem = (0, util_1.getBinaryTreeStem)(this.hashFunction, address, 0);
                    const basicDataBytes = (0, util_1.encodeBinaryTreeLeafBasicData)(account);
                    await this._tree.put(stem, [util_1.BinaryTreeLeafType.BasicData, util_1.BinaryTreeLeafType.CodeHash], [basicDataBytes, account.codeHash]);
                }
                else {
                    // Delete account
        ...
        this.deleteAccount = async (address) => {
            ...
            if (this._caches?.account === undefined) {
                const stem = (0, util_1.getBinaryTreeStem)(this.hashFunction, address);
                // Special instance where we delete the account and revert the trie value to untouched
                await this._tree.put(stem, [util_1.BinaryTreeLeafType.BasicData, util_1.BinaryTreeLeafType.CodeHash], [null, null]);
            }
        };
        ...
        this.putCode = async (address, value) => {
            ...
            const codeChunks = (0, util_1.chunkifyBinaryTreeCode)(value);
            const chunkStems = (0, util_1.generateBinaryTreeCodeStems)(codeChunks.length, address, this.hashFunction);
            const chunkSuffixes = (0, util_1.generateBinaryTreeChunkSuffixes)(codeChunks.length);
            // Put the code chunks corresponding to the first stem (up to 128 chunks)
            await this._tree.put(chunkStems[0], chunkSuffixes.slice(0, chunkSuffixes.length <= util_1.BINARY_TREE_CODE_OFFSET
                ? chunkSuffixes.length
                : util_1.BINARY_TREE_CODE_OFFSET), codeChunks.slice(0, codeChunks.length <= util_1.BINARY_TREE_CODE_OFFSET ? codeChunks.length : util_1.BINARY_TREE_CODE_OFFSET));
            // Put additional chunks under additional stems as applicable
            for (let stem = 1; stem < chunkStems.length; stem++) {
                const sliceStart = util_1.BINARY_TREE_CODE_OFFSET + util_1.BINARY_TREE_NODE_WIDTH * (stem - 1);
                const sliceEnd = value.length <= util_1.BINARY_TREE_CODE_OFFSET + util_1.BINARY_TREE_NODE_WIDTH * stem
                    ? value.length
                    : util_1.BINARY_TREE_CODE_OFFSET + util_1.BINARY_TREE_NODE_WIDTH * stem;
        ...
        this.getCode = async (address) => {
            ...
            const stems = (0, util_1.generateBinaryTreeCodeStems)(Math.ceil(codeSize / util_1.BINARY_TREE_CODE_CHUNK_SIZE), address, this.hashFunction);
            const chunkSuffixes = (0, util_1.generateBinaryTreeChunkSuffixes)(Math.ceil(codeSize / util_1.BINARY_TREE_CODE_CHUNK_SIZE));
            const chunksByStem = new Array(stems.length);
            // Retrieve the code chunks stored in the first leaf node
            chunksByStem[0] = await this._tree.get(stems[0], chunkSuffixes.slice(0, codeSize <= util_1.BINARY_TREE_CODE_OFFSET ? codeSize : util_1.BINARY_TREE_CODE_OFFSET));
            // Retrieve code chunks on any additional stems
            for (let stem = 1; stem < stems.length; stem++) {
                const sliceStart = util_1.BINARY_TREE_CODE_OFFSET + util_1.BINARY_TREE_NODE_WIDTH * (stem - 1);
                const sliceEnd = codeSize <= util_1.BINARY_TREE_CODE_OFFSET + util_1.BINARY_TREE_NODE_WIDTH * stem
                    ? codeSize
                    : util_1.BINARY_TREE_CODE_OFFSET + util_1.BINARY_TREE_NODE_WIDTH * stem;
            ...
            for (let x = 0; x < chunks.length; x++) {
                if (chunks[x] === undefined)
                    throw (0, util_1.EthereumJSErrorWithoutCode)(`expected code chunk at index ${x}, got undefined`);
                let lastChunkByteIndex = util_1.BINARY_TREE_CODE_CHUNK_SIZE;
                ...
                if (x === chunks.length - 1) {
                    ...
                    lastChunkByteIndex = codeSize % util_1.BINARY_TREE_CODE_CHUNK_SIZE || util_1.BINARY_TREE_CODE_CHUNK_SIZE;
                }
                code.set(chunks[x].slice(1, lastChunkByteIndex + 1), code.byteOffset + x * util_1.BINARY_TREE_CODE_CHUNK_SIZE);
            }
            this._caches?.code?.put(address, code);
            return code;
        };
        this.getCodeSize = async (address) => {
            const accountBytes = (await this._tree.get((0, util_1.getBinaryTreeStem)(this.hashFunction, address), [
                util_1.BinaryTreeLeafType.BasicData,
            ]))[0];
            if (accountBytes === null)
                return 0;
            return (0, util_1.decodeBinaryTreeLeafBasicData)(accountBytes).codeSize;
        };
        this.getStorage = async (address, key) => {
            ...
            const storageKey = (0, util_1.getBinaryTreeKeyForStorageSlot)(address, (0, util_1.bytesToBigInt)(key), this.hashFunction);
        ...
        this.putStorage = async (address, key, value) => {
            ...
            if (this._caches?.storage === undefined) {
                const storageKey = (0, util_1.getBinaryTreeKeyForStorageSlot)(address, (0, util_1.bytesToBigInt)(key), this.hashFunction);
                await this._tree.put(storageKey.slice(0, 31), [storageKey[31]], [(0, util_1.setLengthLeft)(value, 32)]);
            }
        };
        ...
        if (opts.common?.isActivatedEIP(7864) === false) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('EIP-7864 required for binary tree state management');
        }
        this.hashFunction = opts.hashFunction ?? blake3_1.blake3;
        this._tree =
            opts.tree ??
                new binarytree_1.BinaryTree({
                    hashFunction: this.hashFunction,
                    db: new util_1.MapDB(),
                    useRootPersistence: false,
                    cacheSize: 0,
                });
        this._debug = (0, debug_1.default)('statemanager:binarytree');
    ...
    initBinaryTreeExecutionWitness(_blockNum, executionWitness) {
        ...
        this.preStateRoot = (0, util_1.hexToBytes)(executionWitness.parentStateRoot); // set prestate root
    ...
    async getComputedValue(accessedState) {
        ...
        switch (type) {
            case common_1.BinaryTreeAccessedStateType.BasicData: {
                if (this._caches === undefined) {
                    const accountData = await this.getAccount(address);
                    if (accountData === undefined) {
                        return null;
                    }
                    const basicDataBytes = (0, util_1.encodeBinaryTreeLeafBasicData)(accountData);
                    ...
                    const basicDataBytes = (0, util_1.encodeBinaryTreeLeafBasicData)((0, util_1.createPartialAccountFromRLP)(encodedAccount));
                    return (0, util_1.bytesToHex)(basicDataBytes);
                }
            }
            case common_1.BinaryTreeAccessedStateType.CodeHash: {
                if (this._caches === undefined) {
                    const accountData = await this.getAccount(address);
                    if (accountData === undefined) {
                        return null;
            ...
            case common_1.BinaryTreeAccessedStateType.Code: {
                ...
                return (0, util_1.bytesToHex)((0, util_1.setLengthLeft)((0, util_1.setLengthRight)(code.slice(codeOffset, codeOffset + util_1.BINARY_TREE_CODE_CHUNK_SIZE), util_1.BINARY_TREE_CODE_CHUNK_SIZE), util_1.BINARY_TREE_CODE_CHUNK_SIZE + 1));
            }
            case common_1.BinaryTreeAccessedStateType.Storage: {
    ...
    async verifyBinaryTreePostState(accessWitness) {
        ...
        for (const accessedState of accessWitness?.accesses() ?? []) {
            const { address, type } = accessedState;
            let extraMeta = '';
            if (accessedState.type === common_1.BinaryTreeAccessedStateType.Code) {
                extraMeta = `codeOffset=${accessedState.codeOffset}`;
            }
            else if (accessedState.type === common_1.BinaryTreeAccessedStateType.Storage) {
            ...
            if (computedValue !== canonicalValue) {
                if (type === common_1.BinaryTreeAccessedStateType.BasicData) {
                    this.DEBUG &&
                        this._debug(`canonical value: `, canonicalValue === null
                            ? null
                            : (0, util_1.decodeBinaryTreeLeafBasicData)((0, util_1.hexToBytes)(canonicalValue)));
                    this.DEBUG &&
                        this._debug(`computed value: `, computedValue === null
                            ? null
                            : (0, util_1.decodeBinaryTreeLeafBasicData)((0, util_1.hexToBytes)(computedValue)));
                }
                this.DEBUG &&
...
exports.StatefulBinaryTreeStateManager = StatefulBinaryTreeStateManager;
