...
class StatelessVerkleStateManager {
    ...
    constructor(opts) {
        ...
        if (opts.common.isActivatedEIP(6800) === false) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('EIP-6800 required for stateless verkle state management');
        }
        ...
        this._debug = (0, debug_1.default)('statemanager:verkle:stateless');
    ...
    async checkChunkWitnessPresent(address, codeOffset) {
        const chunkId = codeOffset / util_1.VERKLE_CODE_CHUNK_SIZE;
    ...
    async getCode(address) {
        ...
        const accessedCode = new Uint8Array(codeSize + util_1.VERKLE_CODE_CHUNK_SIZE);
        const chunks = Math.floor(codeSize / util_1.VERKLE_CODE_CHUNK_SIZE) + 1;
        for (let chunkId = 0; chunkId < chunks; chunkId++) {
            ...
            const codeOffset = chunkId * util_1.VERKLE_CODE_CHUNK_SIZE;
            ...
            if (codeChunk !== undefined) {
                ...
                // end up here
                accessedCode.fill(0xfe, codeOffset, util_1.VERKLE_CODE_CHUNK_SIZE);
            }
        }
    ...
    async getCodeSize(address) {
        ...
        if (elem !== undefined) {
            const account = elem.accountRLP !== undefined ? (0, util_1.createPartialAccountFromRLP)(elem.accountRLP) : undefined;
    ...
    async getStorage(address, key) {
        ...
        const rawStorageValue = this._state[(0, util_1.bytesToHex)(storageKey)];
        const storageValue = rawStorageValue === null ? new Uint8Array() : (0, util_1.hexToBytes)(rawStorageValue);
        this._caches?.storage?.put(address, key, storageValue ?? (0, util_1.hexToBytes)('0x80'));
        return storageValue;
    }
    /**
    ...
    async getAccount(address) {
        ...
        const stem = (0, util_1.getVerkleStem)(this.verkleCrypto, address, 0);
        const basicDataKey = (0, util_1.getVerkleKey)(stem, util_1.VerkleLeafType.BasicData);
        const codeHashKey = (0, util_1.getVerkleKey)(stem, util_1.VerkleLeafType.CodeHash);
        const basicDataRaw = this._state[(0, util_1.bytesToHex)(basicDataKey)];
        ...
        const { version, balance, nonce, codeSize } = (0, util_1.decodeVerkleLeafBasicData)((0, util_1.hexToBytes)(basicDataRaw));
    ...
    async putAccount(address, account) {
        ...
        if (this._caches?.account === undefined) {
            ...
            this._state[(0, util_1.bytesToHex)(basicDataKey)] = (0, util_1.bytesToHex)(basicDataBytes);
        }
        else {
            if (account !== undefined) {
                this._caches?.account?.put(address, account, true);
    ...
    async getStateRoot() {
        if (this._cachedStateRoot === undefined) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Cache state root missing');
        }
