...
exports.DefaultStateManager = exports.CODEHASH_PREFIX = void 0;
...
class DefaultStateManager {
    ...
    constructor(opts = {}) {
        ...
        this._trie = opts.trie ?? new trie_1.Trie({ useKeyHashing: true, common: this.common });
        this._storageTries = {};
    ...
    async getContractStorage(address, key) {
        ...
        if (!this._storageCacheSettings.deactivate) {
            this._storageCache?.put(address, key, value ?? (0, util_1.hexToBytes)('0x80'));
        }
        const decoded = rlp_1.RLP.decode(value ?? new Uint8Array(0));
        return decoded;
    }
    ...
    async flush() {
        ...
        if (!this._storageCacheSettings.deactivate) {
            ...
            for (const item of items) {
                const address = util_1.Address.fromString(`0x${item[0]}`);
                const keyHex = item[1];
    ...
    async getProof(address, storageSlots = []) {
        ...
        if (!account) {
            ...
            const returnValue = {
                ...
                accountProof: (await this._trie.createProof(address.bytes)).map((p) => (0, util_1.bytesToHex)(p)),
                storageProof: [],
            };
            return returnValue;
        }
        const accountProof = (await this._trie.createProof(address.bytes)).map((p) => (0, util_1.bytesToHex)(p));
        const storageProof = [];
        const storageTrie = this._getStorageTrie(address, account);
        for (const storageKey of storageSlots) {
            const proof = (await storageTrie.createProof(storageKey)).map((p) => (0, util_1.bytesToHex)(p));
            const value = (0, util_1.bytesToHex)(await this.getContractStorage(address, storageKey));
            const proofItem = {
        ...
        if (Array.isArray(proof)) {
            if (proof.length === 0) {
                ...
                const trie = opts.trie ??
                    (await trie_1.Trie.createFromProof(proof[0].accountProof.map((e) => (0, util_1.hexToBytes)(e)), { useKeyHashing: true }));
                const sm = new DefaultStateManager({ ...opts, trie });
                const address = util_1.Address.fromString(proof[0].address);
    ...
    async addStorageProof(storageProof, storageHash, address, safe = false) {
        ...
        for (let i = 0; i < storageProof.length; i++) {
            await trie.updateFromProof(storageProof[i].proof.map((e) => (0, util_1.hexToBytes)(e)), safe);
        }
    }
    /**
    ...
    async addProofData(proof, safe = false) {
        if (Array.isArray(proof)) {
            for (let i = 0; i < proof.length; i++) {
                await this._trie.updateFromProof(proof[i].accountProof.map((e) => (0, util_1.hexToBytes)(e)), safe);
                await this.addStorageProof(proof[i].storageProof, proof[i].storageHash, util_1.Address.fromString(proof[i].address), safe);
            }
        }
        else {
    ...
    async verifyProof(proof) {
        ...
        const value = await trie_1.Trie.verifyProof(key, accountProof, {
            useKeyHashing: true,
        });
        if (value === null) {
        ...
        for (const stProof of proof.storageProof) {
            ...
            const proofValue = await trie_1.Trie.verifyProof(storageKey, storageProof, {
                useKeyHashing: true,
            });
    ...
    async generateCanonicalGenesis(initState) {
        ...
        for (const address of addresses) {
            ...
            if (!Array.isArray(state)) {
                ...
                const account = util_1.Account.fromAccountData({ balance, nonce });
                await this.putAccount(addr, account);
                if (code !== undefined) {
                    await this.putContractCode(addr, (0, util_1.toBytes)(code));
                }
                if (storage !== undefined) {
                    for (const [key, value] of storage) {
                        await this.putContractStorage(addr, (0, util_1.toBytes)(key), (0, util_1.toBytes)(value));
                    }
                }
            }
        }
