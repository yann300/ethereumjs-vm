...
import { Account, Address, KECCAK256_NULL, KECCAK256_NULL_S, KECCAK256_RLP, KECCAK256_RLP_S, bigIntToHex, bytesToBigInt, bytesToHex, bytesToUnprefixedHex, concatBytes, equalsBytes, hexToBytes, setLengthLeft, short, toBytes, unpadBytes, unprefixedHexToBytes, utf8ToBytes, } from '@ethereumjs/util';
...
export class DefaultStateManager {
        ...
        if (Array.isArray(proof)) {
            if (proof.length === 0) {
                ...
                const trie = opts.trie ??
                    (await Trie.createFromProof(proof[0].accountProof.map((e) => hexToBytes(e)), { useKeyHashing: true }));
    ...
    async addStorageProof(storageProof, storageHash, address, safe = false) {
        ...
        for (let i = 0; i < storageProof.length; i++) {
            await trie.updateFromProof(storageProof[i].proof.map((e) => hexToBytes(e)), safe);
        }
    }
    /**
    ...
    async addProofData(proof, safe = false) {
        if (Array.isArray(proof)) {
            for (let i = 0; i < proof.length; i++) {
                await this._trie.updateFromProof(proof[i].accountProof.map((e) => hexToBytes(e)), safe);
