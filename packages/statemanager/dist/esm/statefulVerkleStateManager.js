...
import { Account, EthereumJSErrorWithoutCode, KECCAK256_NULL, MapDB, VERKLE_CODE_CHUNK_SIZE, VERKLE_CODE_OFFSET, VERKLE_NODE_WIDTH, VerkleLeafType, bigIntToBytes, bytesToBigInt, bytesToHex, chunkifyCode, createAddressFromString, createPartialAccount, createPartialAccountFromRLP, decodeVerkleLeafBasicData, encodeVerkleLeafBasicData, equalsBytes, generateChunkSuffixes, generateCodeStems, getVerkleStem, getVerkleTreeKeyForStorageSlot, hexToBigInt, hexToBytes, padToEven, setLengthLeft, setLengthRight, short, unprefixedHexToBytes, } from '@ethereumjs/util';
...
export class StatefulVerkleStateManager {
    constructor(opts) {
        ...
        this.putCode = async (address, value) => {
            ...
            await this._trie.put(chunkStems[0], chunkSuffixes.slice(0, chunkSuffixes.length <= VERKLE_CODE_OFFSET ? chunkSuffixes.length : VERKLE_CODE_OFFSET), codeChunks.slice(0, codeChunks.length <= VERKLE_CODE_OFFSET ? codeChunks.length : VERKLE_CODE_OFFSET));
        ...
        this.getCode = async (address) => {
            ...
            for (let x = 0; x < chunks.length; x++) {
                ...
                code.set(chunks[x].slice(1, lastChunkByteIndex + 1), code.byteOffset + x * VERKLE_CODE_CHUNK_SIZE);
            }
        ...
        this.getStorage = async (address, key) => {
            ...
            const storageKey = await getVerkleTreeKeyForStorageSlot(address, bytesToBigInt(key), this.verkleCrypto);
    ...
    async getComputedValue(accessedState) {
        ...
        switch (type) {
            ...
            case VerkleAccessedStateType.Code: {
                ...
                return bytesToHex(setLengthLeft(setLengthRight(code.slice(codeOffset, codeOffset + VERKLE_CODE_CHUNK_SIZE), VERKLE_CODE_CHUNK_SIZE), VERKLE_CODE_CHUNK_SIZE + 1));
            }
