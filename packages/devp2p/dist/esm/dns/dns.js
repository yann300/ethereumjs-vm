...
export class DNS {
    ...
    _getEntryType(entry) {
        if (entry.startsWith(ENR.ROOT_PREFIX))
            return ENR.ROOT_PREFIX;
        if (entry.startsWith(ENR.BRANCH_PREFIX))
    ...
    async _getTXTRecord(subdomain, context) {
        ...
        if (response[0].length === 0)
            throw EthereumJSErrorWithoutCode('Received empty TXT record');
