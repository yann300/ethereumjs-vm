...
import { EthereumJSErrorWithoutCode, bigIntToBytes, bytesToInt, concatBytes, hexToBytes, intToBytes, setLengthLeft, } from '@ethereumjs/util';
...
export class ECIES {
    ...
    _decryptMessage(data, sharedMacData = null) {
        assertEq(data.subarray(0, 1), hexToBytes('0x04'), 'wrong ecies header (possible cause: EIP8 upgrade)', debug);
    ...
    createAuthEIP8() {
        ...
        const data = [
            concatBytes(setLengthLeft(bigIntToBytes(sig.r), 32), setLengthLeft(bigIntToBytes(sig.s), 32), Uint8Array.from([sig.recovery])),
    ...
    createAuthNonEIP8() {
        ...
        const x = ecdhX(this._remotePublicKey, this._privateKey);
        const sig = this._ecdsaSign(xor(x, this._nonce), this._ephemeralPrivateKey);
        const data = concatBytes(bigIntToBytes(sig.r), bigIntToBytes(sig.s), Uint8Array.from([sig.recovery]), this._keccakFunction(pk2id(this._ephemeralPublicKey)), pk2id(this._publicKey), this._nonce, Uint8Array.from([0x00]));
    ...
    parseAuthPlain(data, sharedMacData = null) {
        ...
        if (!this._gotEIP8Auth) {
            ...
            const decoded = unstrictDecode(decrypted);
        ...
        if (heId !== null && this._remoteEphemeralPublicKey !== null) {
            assertEq(this._keccakFunction(pk2id(this._remoteEphemeralPublicKey)), heId, 'the hash of the ephemeral key should match', debug);
        }
    }
    parseAuthEIP8(data) {
    ...
    parseBody(data) {
        if (this._bodySize === null)
            throw EthereumJSErrorWithoutCode('need to parse header first');
