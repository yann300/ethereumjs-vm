...
import { EthereumJSErrorWithoutCode, bytesToHex, bytesToUnprefixedHex, concatBytes, equalsBytes, } from '@ethereumjs/util';
...
export function zfill(bytes, size, leftpad = true) {
...
export function assertEq(expected, actual, msg, debug, messageName) {
    let fullMsg;
    if (expected instanceof Uint8Array && actual instanceof Uint8Array) {
        if (equalsBytes(expected, actual))
            return;
...
export function formatLogId(id, verbose) {
    const numChars = 7;
    if (verbose) {
        return id;
    }
...
export function formatLogData(data, verbose) {
    const maxChars = 60;
...
export function unstrictDecode(value) {
...
export const ipToString = (bytes, offset, length) => {
...
export const isV4Format = function (ip) {
    return ipv4Regex.test(ip);
};
export const isV6Format = function (ip) {
    return ipv6Regex.test(ip);
};
export const ipToBytes = (ip, bytes, offset = 0) => {
    offset = ~~offset;
    let result;
    if (isV4Format(ip)) {
