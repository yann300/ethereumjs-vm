...
class Server {
    constructor(dpt, privateKey, options) {
        ...
        this._requestsCache = new lru_cache_1.LRUCache({ max: 1000, ttl: 1000 }); // 1 sec * 1000
    ...
    async ping(peer) {
        ...
        const rKey = (0, util_1.bytesToUnprefixedHex)(hash);
        this._requests.set(rKey, {
            ...
            timeoutId: setTimeout(() => {
                if (this._requests.get(rKey) !== undefined) {
                    if (this.DEBUG) {
                        this._debug(`ping timeout: ${peer.address}:${peer.udpPort} ${peer.id ? (0, util_ts_1.formatLogId)((0, util_1.bytesToHex)(peer.id), verbose) : '-'}`);
                    }
                    this._requests.delete(rKey);
                    deferred.reject((0, util_1.EthereumJSErrorWithoutCode)(`Timeout error: ping ${peer.address}:${peer.udpPort}`));
                }
                else {
    ...
    _send(peer, typename, data) {
        if (this.DEBUG) {
            this.debug(typename, `send ${typename} to ${peer.address}:${peer.udpPort} (peerId: ${peer.id ? (0, util_ts_1.formatLogId)((0, util_1.bytesToHex)(peer.id), verbose) : '-'})`);
        }
        const msg = (0, message_ts_1.encode)(typename, data, this._privateKey, this._common);
    ...
    _handler(msg, rinfo) {
        const info = (0, message_ts_1.decode)(msg, this._common); // Dgram serializes everything to `Uint8Array`
        const peerId = (0, util_ts_1.pk2id)(info.publicKey);
        if (this.DEBUG) {
            this.debug(info.typename.toString(), `received ${info.typename} from ${rinfo.address}:${rinfo.port} (peerId: ${(0, util_ts_1.formatLogId)((0, util_1.bytesToHex)(peerId), verbose)})`);
        }
        ...
        switch (info.typename) {
            ...
            case 'pong': {
                const rKey = (0, util_1.bytesToUnprefixedHex)(info.data.hash);
