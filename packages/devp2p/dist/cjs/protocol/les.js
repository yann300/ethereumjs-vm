...
class LES extends protocol_js_1.Protocol {
    constructor(version, peer, send) {
        super(peer, send, types_js_1.ProtocolType.LES, version, LES.MESSAGE_CODES);
        this._status = null;
        this._peerStatus = null;
        this._statusTimeoutId = setTimeout(() => {
            this._peer.disconnect(types_js_1.DISCONNECT_REASON.TIMEOUT);
        }, 5000); // 5 sec * 1000
        this.DEBUG =
        ...
        switch (code) {
            case LES.MESSAGE_CODES.STATUS: {
                ...
                for (const value of payload) {
                    status[(0, util_1.bytesToUtf8)(value[0])] = value[1];
                }
    ...
    _handleStatus() {
        ...
        (0, util_js_1.assertEq)(this._status['protocolVersion'], this._peerStatus['protocolVersion'], 'Protocol version mismatch', this.debug.bind(this), 'STATUS');
        (0, util_js_1.assertEq)(this._status['networkId'], this._peerStatus['networkId'], 'NetworkId mismatch', this.debug.bind(this), 'STATUS');
        (0, util_js_1.assertEq)(this._status['genesisHash'], this._peerStatus['genesisHash'], 'Genesis block mismatch', this.debug.bind(this), 'STATUS');
    ...
    _getStatusString(status) {
        let sStr = `[V:${(0, util_1.bytesToInt)(status['protocolVersion'])}, `;
        sStr += `NID:${(0, util_1.bytesToInt)(status['networkId'])}, HTD:${(0, util_1.bytesToInt)(status['headTd'])}, `;
        sStr += `HeadH:${(0, util_1.bytesToHex)(status['headHash'])}, HeadN:${(0, util_1.bytesToInt)(status['headNum'])}, `;
        sStr += `GenH:${(0, util_1.bytesToHex)(status['genesisHash'])}`;
        if (status['serveHeaders'] !== undefined)
            sStr += `, serveHeaders active`;
        if (status['serveChainSince'] !== undefined)
            sStr += `, ServeCS: ${(0, util_1.bytesToInt)(status['serveChainSince'])}`;
        if (status['serveStateSince'] !== undefined)
            sStr += `, ServeSS: ${(0, util_1.bytesToInt)(status['serveStateSince'])}`;
        ...
        if (status['forkID'] !== undefined)
            sStr += `, forkID: [crc32: ${(0, util_1.bytesToHex)(status['forkID'][0])}, nextFork: ${(0, util_1.bytesToInt)(status['forkID'][1])}]`;
        if (status['recentTxLookup'] !== undefined)
            sStr += `, recentTxLookup: ${(0, util_1.bytesToInt)(status['recentTxLookup'])}`;
        sStr += `]`;
        return sStr;
    }
    sendStatus(status) {
        ...
        if (status.announceType === undefined) {
            status['announceType'] = (0, util_1.intToBytes)(exports.DEFAULT_ANNOUNCE_TYPE);
        }
        status['protocolVersion'] = (0, util_1.intToBytes)(this._version);
        status['networkId'] = (0, util_1.bigIntToBytes)(this._peer.common.chainId());
        this._status = status;
        const statusList = [];
        for (const key of Object.keys(status)) {
            statusList.push([(0, util_1.utf8ToBytes)(key), status[key]]);
        }
        if (this.DEBUG) {
            this.debug('STATUS', 
            // @ts-ignore
        ...
        let payload = rlp_1.RLP.encode(statusList);
    ...
    sendMessage(code, payload) {
        if (this.DEBUG) {
            ...
            // @ts-ignore
            this._peer._socket.remotePort}: ${(0, util_js_1.formatLogData)((0, util_1.bytesToHex)(rlp_1.RLP.encode(payload)), this._verbose)}`);
        }
        switch (code) {
            case LES.MESSAGE_CODES.STATUS:
