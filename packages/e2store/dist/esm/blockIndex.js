...
import { CommonTypes, VERSION } from "./types.js";
export function getBlockIndex(bytes) {
    ...
    if (!equalsBytes(type, CommonTypes.BlockIndex)) {
        throw EthereumJSErrorWithoutCode(`Expected block index (type: ${CommonTypes.BlockIndex}) but got ${type}`);
    }
    return { data, type, count, recordStart };
}
export function readBlockIndex(data, count) {
...
export async function createBlockIndex(blockTuples, startingNumber) {
    ...
    const offsets = offsetBigInt.map((o) => bigInt64ToBytes(o, true));
