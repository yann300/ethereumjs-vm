...
import { EthereumJSErrorWithoutCode, bigInt64ToBytes, bytesToHex, concatBytes, equalsBytes, } from '@ethereumjs/util';
import { uint256 } from 'micro-eth-signer/ssz';
import { compressData, decompressData } from "./snappy.js";
import { CommonTypes, Era1Types, EraTypes } from "./types.js";
 ...
 */
export const readEntry = (bytes) => {
    if (bytes.length < 8)
        throw EthereumJSErrorWithoutCode(`invalid data length, got ${bytes.length}, expected at least 8`);
    ...
    if (length > bytes.length) {
        // Check for overflow
        throw EthereumJSErrorWithoutCode(`invalid data length, got ${length}, expected max of ${bytes.length - 8}`);
    }
 ...
 */
export const formatEntry = async ({ type, data, }) => {
