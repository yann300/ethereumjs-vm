 ...
 */
export const readSlotIndex = (bytes) => {
 ...
 */
export const getEraIndexes = (eraContents) => {
 ...
 */
export const readBeaconState = async (eraData) => {
    const indices = getEraIndexes(eraData);
    const stateEntry = readEntry(eraData.slice(indices.stateSlotIndex.recordStart + indices.stateSlotIndex.slotOffsets[0]));
 ...
 */
export const readBeaconBlock = async (eraData, offset) => {
    const indices = getEraIndexes(eraData);
    const blockEntry = readEntry(eraData.slice(indices.blockSlotIndex.recordStart + indices.blockSlotIndex.slotOffsets[offset]));
    const data = await parseEntry(blockEntry);
    if (equalsBytes(blockEntry.type, EraTypes.CompressedSignedBeaconBlockType) === false) {
        throw EthereumJSErrorWithoutCode(`expected CompressedSignedBeaconBlockType type, got ${bytesToHex(blockEntry.type)}`);
    }
 ...
 */
export async function* readBlocksFromEra(eraFile) {
