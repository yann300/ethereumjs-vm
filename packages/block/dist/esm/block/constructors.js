...
import { createTx, createTxFromBlockBodyData, createTxFromRLP, normalizeTxParams, } from '@ethereumjs/tx';
import { EthereumJSErrorWithoutCode, bigIntToHex, bytesToHex, bytesToUtf8, createWithdrawal, equalsBytes, fetchFromProvider, getProvider, hexToBytes, intToHex, isHexString, } from '@ethereumjs/util';
...
export function createBlockFromBytesArray(values, opts) {
    ...
    if (header.common.isActivatedEIP(6800) && executionWitnessBytes === undefined) {
        throw EthereumJSErrorWithoutCode('Invalid serialized block input: EIP-6800 is active, and execution witness is undefined');
    }
    // parse transactions
    const transactions = [];
    for (const txData of txsData ?? []) {
