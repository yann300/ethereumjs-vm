...
import { BIGINT_0, BIGINT_1, EthereumJSErrorWithoutCode, TypeOutput, concatBytes, isHexString, toType, } from '@ethereumjs/util';
/**
...
export function valuesArrayToHeaderData(values) {
    ...
    if (values.length > 21) {
        throw EthereumJSErrorWithoutCode(`invalid header. More values than expected were received. Max: 20, got: ${values.length}`);
    }
    if (values.length < 15) {
        throw EthereumJSErrorWithoutCode(`invalid header. Less values than expected were received. Min: 15, got: ${values.length}`);
    }
    return {
        parentHash,
        uncleHash,
 ...
 * @param common
 */
export const computeBlobGasPrice = (excessBlobGas, common) => {
    return fakeExponential(common.param('minBlobGas'), excessBlobGas, common.param('blobGasPriceUpdateFraction'));
};
/**
 ...
 */
export async function genWithdrawalsTrieRoot(wts, emptyTrie) {
 ...
 * @param optional emptyTrie to use to generate the root
 */
export async function genTransactionsTrieRoot(txs, emptyTrie) {
    const trie = emptyTrie ?? new MerklePatriciaTrie();
 ...
 */
export function genRequestsRoot(requests, sha256Function) {
