...
import { BIGINT_0, EthereumJSErrorWithoutCode, KeyEncoding, Lock, MapDB, RLP_EMPTY_STRING, ValueEncoding, bytesToBigInt, bytesToHex, bytesToUnprefixedHex, bytesToUtf8, concatBytes, equalsBytes, } from '@ethereumjs/util';
...
export class MerklePatriciaTrie {
    ...
    async findPath(key, throwIfMissing = false, partialPath = {
        ...
        for (let i = 0; i < partialPath.stack.length - 1; i++) {
            ...
            progress += stack[i] instanceof BranchMPTNode ? 1 : stack[i].keyLength();
        }
                ...
                await this.walkTrie(this.root(), async function (_, node, key, controller) {
                    ...
                    if (node instanceof BranchMPTNode) {
                        for (const item of node._branches) {
                            ...
                            if (item !== null &&
                                bytesToUnprefixedHex(isRawMPTNode(item) ? controller.trie.appliedKey(RLP.encode(item)) : item) === dbkey) {
                                found = true;
                                return;
                            }
                        }
