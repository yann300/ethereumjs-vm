import { EthereumJSErrorWithoutCode, Lock, bitsToBytes, bytesToBits, bytesToHex, concatBytes, equalsBits, equalsBytes, matchingBitsLength, setLengthRight, } from '@ethereumjs/util';
...
export class BinaryTree {
    ...
    async put(stem, suffixes, values) {
        ...
        if (values.length > 0 && values.length !== suffixes.length)
            throw EthereumJSErrorWithoutCode(`expected number of values (${values.length}) to equal number of suffixes (${suffixes.length})`);
        ...
        for (let i = 0; i < suffixes.length; i++) {
            ...
            this.DEBUG &&
                this.debug(`Setting value for suffix: ${suffix} to value: ${value instanceof Uint8Array ? bytesToHex(value) : value} at stem node with stem: ${bytesToHex(stem)}`, ['put']);
        }
        ...
        while (foundPath.stack.length > 1) {
            ...
            if (isInternalBinaryNode(node)) {
                ...
                throw EthereumJSErrorWithoutCode(`Expected internal node at path ${path.join(',')}, got ${node}`);
            }
        }
    ...
    async findPath(keyInBytes) {
        ...
        while (!finished) {
            ...
            if (matchingKeyLength === childNode.path.length &&
                ...
                if (matchingKeyLength === keyInBits.length &&
                    ...
                    this.DEBUG &&
                        this.debug(`Path ${bytesToHex(keyInBytes)} - found full path to node ${bytesToHex(this.merkelize(decodedNode))}.`, ['find_path']);
            ...
            this.DEBUG &&
                this.debug(`Partial Path ${keyInBits.slice(0, matchingKeyLength)} - found next node in path ${bytesToHex(this.merkelize(decodedNode))}.`, ['find_path']);
        ...
        this.DEBUG &&
            this.debug(`Found partial path ${bytesToHex(bitsToBytes(keyInBits.slice(256 - result.remaining.length)))} but sought node is not present in trie.`, ['find_path']);
        return result;
    }
    /**
    ...
    merkelize(node) {
        ...
        if (isInternalBinaryNode(node)) {
            ...
            return this.hash(concatBytes(leftChild === null ? this.hash(null) : leftChild.hash, rightChild === null ? this.hash(null) : rightChild.hash));
        }
        ...
        while (currentLayerHashes.length > 1) {
            ...
            for (let i = 0; i < currentLayerHashes.length; i += 2) {
                newLayerHashes.push(this.hash(concatBytes(currentLayerHashes[i], currentLayerHashes[i + 1])));
            }
