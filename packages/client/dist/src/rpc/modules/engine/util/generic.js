...
exports.validateHardforkRange = exports.validateTerminalBlock = exports.validHash = exports.validExecutedChainBlock = exports.recursivelyFindParents = void 0;
 ...
 */
const recursivelyFindParents = async (vmHeadHash, parentHash, chain) => {
    if ((0, util_1.equalsBytes)(parentHash, vmHeadHash) || (0, util_1.equalsBytes)(parentHash, new Uint8Array(32))) {
        return [];
    }
    ...
    while (!(0, util_1.equalsBytes)(parentBlocks[parentBlocks.length - 1].hash(), vmHeadHash)) {
...
exports.recursivelyFindParents = recursivelyFindParents;
/**
 * Returns the block hash as a 0x-prefixed hex string if found valid in the blockchain, otherwise returns null.
 */
const validExecutedChainBlock = async (blockOrHash, chain) => {
    try {
        const block = blockOrHash instanceof block_1.Block ? blockOrHash : await chain.getBlock(blockOrHash);
        ...
        if (vmHead.header.number >= block.header.number) {
            ...
            if (canonicalHash instanceof Uint8Array && (0, util_1.equalsBytes)(block.hash(), canonicalHash)) {
                return block;
            }
        }
...
exports.validExecutedChainBlock = validExecutedChainBlock;
/**
 * Returns the block hash as a 0x-prefixed hex string if found valid in the blockchain, otherwise returns null.
 */
const validHash = async (hash, chain, chainCache) => {
    ...
    try {
        ...
        for (let inspectedParents = 0; inspectedParents < maxDepth; inspectedParents++) {
            const unPrefixedHashStr = (0, util_1.bytesToUnprefixedHex)(hash);
            validParent =
            ...
            const isBlockExecuted = (executedBlocks.get(unPrefixedHashStr) ?? (await (0, exports.validExecutedChainBlock)(hash, chain))) !==
                null;
            if (isBlockExecuted) {
                return (0, util_1.bytesToHex)(hash);
            }
            else {
 ...
 */
const validateTerminalBlock = async (block, chain) => {
    const ttd = chain.config.chainCommon.hardforkTTD(common_1.Hardfork.Paris);
    if (ttd === null)
        return false;
    ...
    if (block.isGenesis())
        return blockTd >= ttd;
    const parentBlockTd = await chain.getTd(block.header.parentHash, block.header.number - util_1.BIGINT_1);
    return blockTd >= ttd && parentBlockTd < ttd;
};
exports.validateTerminalBlock = validateTerminalBlock;
function validateHardforkRange(chainCommon, methodVersion, checkNotBeforeHf, checkNotAfterHf, timestamp) {
...
exports.validateHardforkRange = validateHardforkRange;
