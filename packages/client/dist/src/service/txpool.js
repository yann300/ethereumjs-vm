...
class TxPool {
    ...
    checkRunState() {
        ...
        const target = (this.config.syncTargetHeight ?? util_1.BIGINT_0) -
    ...
    validateTxGasBump(existingTx, addedTx) {
        ...
        if (newGasPrice.tip < minTipCap || newGasPrice.maxFee < minFeeCap) {
            throw new Error(`replacement gas too low, got tip ${newGasPrice.tip}, min: ${minTipCap}, got fee ${newGasPrice.maxFee}, min: ${minFeeCap}`);
        }
        if (addedTx instanceof tx_1.BlobEIP4844Transaction && existingTx instanceof tx_1.BlobEIP4844Transaction) {
    ...
    async validate(tx, isLocalTransaction = false) {
        ...
        if (inPool) {
            ...
            if (existingTxn) {
                if ((0, util_1.equalsBytes)(existingTxn.tx.hash(), tx.hash())) {
                    throw new Error(`${(0, util_1.bytesToHex)(tx.hash())}: this transaction is already in the TxPool`);
                }
                this.validateTxGasBump(existingTxn.tx, tx);
            }
        }
        const block = await this.service.chain.getCanonicalHeadHeader();
        if (typeof block.baseFeePerGas === 'bigint' && block.baseFeePerGas !== util_1.BIGINT_0) {
            if (currentGasPrice.maxFee < block.baseFeePerGas / util_1.BIGINT_2 && !isLocalTransaction) {
                throw new Error(`Tx cannot pay basefee of ${block.baseFeePerGas}, have ${currentGasPrice.maxFee} (not within 50% range of current basefee)`);
            }
        }
        if (tx.gasLimit > block.gasLimit) {
            throw new Error(`Tx gaslimit of ${tx.gasLimit} exceeds block gas limit of ${block.gasLimit} (exceeds last block gas limit)`);
        }
        ...
        if (account.nonce > tx.nonce) {
            throw new Error(`0x${sender} tries to send a tx with nonce ${tx.nonce}, but account has nonce ${account.nonce} (tx nonce too low)`);
        }
        const minimumBalance = tx.value + currentGasPrice.maxFee * tx.gasLimit;
        if (account.balance < minimumBalance) {
            throw new Error(`0x${sender} does not have enough balance to cover transaction costs, need ${minimumBalance}, but have ${account.balance} (insufficient balance)`);
        }
    }
    /**
    ...
    async add(tx, isLocalTransaction = false) {
        const hash = (0, util_1.bytesToUnprefixedHex)(tx.hash());
    ...
    getByHash(txHashes) {
        ...
        for (const txHash of txHashes) {
            const txHashStr = (0, util_1.bytesToUnprefixedHex)(txHash);
    ...
    addToKnownByPeer(txHashes, peer) {
        ...
        for (const hash of txHashes) {
            const inSent = this.knownByPeer
                .get(peer.id)
                .filter((sentObject) => sentObject.hash === (0, util_1.bytesToUnprefixedHex)(hash)).length;
            if (inSent === 0) {
                const added = Date.now();
                const add = {
                    hash: (0, util_1.bytesToUnprefixedHex)(hash),
                    added,
                };
            ...
            if (hashesToSend.length > 0) {
                if (peer.eth !== undefined &&
                    ...
                    for (const hash of hashesToSend) {
                        const index = txs[2].findIndex((el) => (0, util_1.equalsBytes)(el, hash));
    ...
    sendTransactions(txs, peers) {
        if (txs.length > 0) {
            ...
            for (const peer of peers) {
                ...
                const newHashesHex = newHashes.map((txHash) => (0, util_1.bytesToUnprefixedHex)(txHash));
                const newTxs = txs.filter((tx) => newHashesHex.includes((0, util_1.bytesToUnprefixedHex)(tx.hash())));
    ...
    markFailedSends(peer, failedHashes, e) {
        for (const txHash of failedHashes) {
            const sendobject = this.knownByPeer
                .get(peer.id)
                ?.filter((sendObject) => sendObject.hash === (0, util_1.bytesToUnprefixedHex)(txHash))[0];
            if (sendobject) {
                sendobject.error = e;
            }
        }
    ...
    async handleAnnouncedTxs(txs, peer, peerPool) {
        ...
        for (const tx of txs) {
            try {
                ...
                this.config.logger.debug(`Error adding tx to TxPool: ${error.message} (tx hash: ${(0, util_1.bytesToHex)(tx.hash())})`);
            }
        }
    ...
    async handleAnnouncedTxHashes(txHashes, peer, peerPool) {
        ...
        const reqHashesStr = reqHashes.map(util_1.bytesToUnprefixedHex);
    ...
    removeNewBlockTxs(newBlocks) {
        ...
        for (const block of newBlocks) {
            for (const tx of block.transactions) {
                const txHash = (0, util_1.bytesToUnprefixedHex)(tx.hash());
    ...
    normalizedGasPrice(tx, baseFee) {
        const supports1559 = tx.supports(tx_1.Capability.EIP1559FeeMarket);
        if (typeof baseFee === 'bigint' && baseFee !== util_1.BIGINT_0) {
            if (supports1559) {
    ...
    txGasPrice(tx) {
        ...
        if ((0, tx_1.isAccessListEIP2930Tx)(tx)) {
            return {
                maxFee: tx.gasPrice,
                tip: tx.gasPrice,
            };
        }
        if ((0, tx_1.isFeeMarketEIP1559Tx)(tx) || (0, tx_1.isBlobEIP4844Tx)(tx)) {
            return {
    ...
    async txsByPriceAndNonce(vm, { baseFee, allowedBlobs } = {}) {
        ...
        for (const [address, poolObjects] of this.pool) {
            ...
            let account = await vm.stateManager.getAccount(new util_1.Address((0, util_1.hexToBytes)('0x' + address)));
            if (account === undefined) {
                account = new util_1.Account();
            }
            const { nonce } = account;
        ...
        const byPrice = new Heap({
            comparBefore: (a, b) => this.normalizedGasPrice(b, baseFee) - this.normalizedGasPrice(a, baseFee) < util_1.BIGINT_0,
        });
        ...
        while (byPrice.length > 0) {
            ...
            if (!(best instanceof tx_1.BlobEIP4844Transaction) ||
        ...
        this.config.logger.info(`txsByPriceAndNonce selected txs=${txs.length}, skipped byNonce=${skippedStats.byNonce} byPrice=${skippedStats.byPrice} byBlobsLimit=${skippedStats.byBlobsLimit}`);
        return txs;
    }
    /**
     * Stop pool execution
