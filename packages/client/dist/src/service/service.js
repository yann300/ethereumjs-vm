...
class Service {
    ...
    constructor(options) {
         ...
         */
        this.STATS_INTERVAL = 1000 * 30; // 30 seconds
        /**
         * Shutdown the client when memory threshold is reached (in percent)
         *
         */
        this.MEMORY_SHUTDOWN_THRESHOLD = 92;
        ...
        this.config.events.on(types_1.Event.PROTOCOL_MESSAGE, async (message, protocol, peer) => {
            if (this.running) {
                try {
        ...
        this.flow = new protocol_1.FlowControl();
    ...
    async open() {
        ...
        this.config.events.on(types_1.Event.POOL_PEER_BANNED, (peer) => this.config.logger.debug(`Peer banned: ${peer}`));
        this.config.events.on(types_1.Event.POOL_PEER_ADDED, (peer) => this.config.logger.debug(`Peer added: ${peer}`));
        this.config.events.on(types_1.Event.POOL_PEER_REMOVED, (peer) => this.config.logger.debug(`Peer removed: ${peer}`));
    ...
    async start() {
        ...
        if (this.v8Engine === null) {
            this.v8Engine = await (0, util_1.getV8Engine)();
        }
