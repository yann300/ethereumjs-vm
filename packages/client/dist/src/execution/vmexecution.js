...
exports.VMExecution = exports.ExecStatus = void 0;
const blockchain_1 = require("@ethereumjs/blockchain");
const common_1 = require("@ethereumjs/common");
const genesis_1 = require("@ethereumjs/genesis");
...
var ExecStatus;
(function (ExecStatus) {
    ExecStatus["VALID"] = "VALID";
    ExecStatus["INVALID"] = "INVALID";
...
class VMExecution extends execution_1.Execution {
    ...
    constructor(options) {
        ...
        this.MAX_TOLERATED_BLOCK_TIME = 12;
        /**
         * Interval for client execution stats output (in ms)
         * for debug log level
         *
         */
        this.STATS_INTERVAL = 1000 * 90; // 90 seconds
        ...
        if (this.metaDB) {
            if (this.config.saveReceipts) {
                this.receiptsManager = new receipt_1.ReceiptsManager({
                    chain: this.chain,
                    config: this.config,
                    metaDB: this.metaDB,
                });
                this.pendingReceipts = new Map();
                this.chain.blockchain.events.addListener('deletedCanonicalBlocks', async (blocks, resolve) => {
            ...
            if (this.config.savePreimages) {
                this.preimagesManager = new preimage_1.PreimagesManager({
                    chain: this.chain,
                    config: this.config,
                    metaDB: this.metaDB,
                });
    ...
    async setupMerkleVM() {
        ...
        const trie = await trie_1.Trie.create({
            db: new level_1.LevelDB(this.stateDB),
            useKeyHashing: true,
            common: this.config.chainCommon,
            cacheSize: this.config.trieCache,
            valueEncoding: this.config.useStringValueTrieDB ? util_1.ValueEncoding.String : util_1.ValueEncoding.Bytes,
        });
        ...
        const stateManager = new statemanager_1.DefaultStateManager({
            ...
            accountCacheOpts: {
                deactivate: false,
                type: statemanager_1.CacheType.LRU,
        ...
        this.merkleVM = await vm_1.VM.create({
    ...
    async setupVerkleVM() {
        ...
        const stateManager = new statemanager_1.StatelessVerkleStateManager();
        this.verkleVM = await vm_1.VM.create({
            common: this.config.execCommon,
            blockchain: this.chain.blockchain,
            stateManager,
            profilerOpts: this.config.vmProfilerOpts,
    ...
    async transitionToVerkle(merkleStateRoot, assignToVM = true) {
        if (this.vm.stateManager instanceof statemanager_1.StatelessVerkleStateManager) {
            return;
        }
        return this.runWithLock(async () => {
            ...
            const verkleStateRoot = await verkleStateManager.getTransitionStateRoot(merkleStateManager, merkleStateRoot);
    ...
    async open() {
        return this.runWithLock(async () => {
            ...
            if (this.config.execCommon.gteHardfork(common_1.Hardfork.Prague)) {
                ...
                this.config.logger.info(`Initializing VM merkle statemanager genesis hardfork=${this.hardfork}`);
                await this.setupMerkleVM();
                this.vm = this.merkleVM;
            }
            ...
            if (number === util_1.BIGINT_0) {
                const genesisState = this.chain['_customGenesisState'] ?? (0, genesis_1.getGenesis)(Number(this.vm.common.chainId()));
                if (!genesisState) {
     ...
     */
    async checkAndReset(headBlock) {
        if (this.chainStatus !== null &&
            (headBlock.header.number > this.chainStatus.height ||
                (0, util_1.equalsBytes)(headBlock.hash(), this.chainStatus?.hash))) {
            return;
        }
        const { number, timestamp, stateRoot } = headBlock.header;
        this.chainStatus = {
        ...
        const td = headBlock.header.number === util_1.BIGINT_0
     ...
     */
    async runWithoutSetHead(opts, receipts, blocking = false, skipBlockchain = false) {
        ...
        await this.runWithLock(async () => {
            try {
                ...
                if (receipts === undefined) {
                    ...
                    const clearCache = !(0, util_1.equalsBytes)(prevVMStateRoot, parentState);
                    ...
                    let td;
                    if (block.common.gteHardfork(common_1.Hardfork.Paris)) {
                        td = this.config.chainCommon.hardforkTTD(common_1.Hardfork.Paris);
                        if (td === null) {
                            throw Error(`Invalid null paris TTD for the chain`);
                        }
                    }
                    else {
                        td = await this.chain.getTd(block.header.parentHash, block.header.number - util_1.BIGINT_1);
                    }
                    ...
                    let vm = this.vm;
                    if (!this.config.execCommon.gteHardfork(common_1.Hardfork.Prague) &&
                        this.config.execCommon.hardforkGteHardfork(hardfork, common_1.Hardfork.Prague)) {
                        // see if this is a transition block
                        const parentBlock = opts?.parentBlock ?? (await this.chain.getBlock(block.header.parentHash));
                        ...
                        if (!this.config.execCommon.hardforkGteHardfork(parentHf, common_1.Hardfork.Prague)) {
                    ...
                    const needsStatelessExecution = vm.stateManager instanceof statemanager_1.StatelessVerkleStateManager;
                    ...
                    const skipHeaderValidation = needsStatelessExecution && this.chain.headers.height < block.header.number - util_1.BIGINT_1;
                ...
                if (receipts !== undefined) {
                    // Save receipts
                    this.pendingReceipts?.set((0, util_1.bytesToHex)(block.hash()), receipts);
                }
                if (!skipBlockchain) {
                    // Bypass updating head by using blockchain db directly
                    const [hash, num] = [block.hash(), block.header.number];
                    const td = (await this.chain.getTd(block.header.parentHash, block.header.number - util_1.BIGINT_1)) +
                        block.header.difficulty;
                    await this.chain.blockchain.dbManager.batch([
                        (0, blockchain_1.DBSetTD)(td, num, hash),
                        ...(0, blockchain_1.DBSetBlockOrHeader)(block),
                        (0, blockchain_1.DBSetHashToNumber)(hash, num),
                        // Skip the op for number to hash to not alter canonical chain
                        ...(0, blockchain_1.DBSaveLookups)(hash, num, true),
                    ]);
                }
            }
            finally {
    ...
    async savePreimages(preimages) {
        if (this.preimagesManager !== undefined) {
            for (const [key, preimage] of preimages) {
                await this.preimagesManager.savePreimage((0, util_1.hexToBytes)(key), preimage);
            }
        }
    }
    /**
     ...
     */
    async setHead(blocks, { finalizedBlock, safeBlock } = {}) {
        if (!this.started || this.config.shutdown)
            return false;
        return this.runWithLock(async () => {
            const vmHeadBlock = blocks[blocks.length - 1];
            const chainPointers = [['vmHeadBlock', vmHeadBlock]];
            ...
            if (!(await this.vm.stateManager.hasStateRoot(vmHeadBlock.header.stateRoot))) {
                ...
                throw Error(`vmHeadBlock's stateRoot not found number=${vmHeadBlock.header.number} root=${(0, util_2.short)(vmHeadBlock.header.stateRoot)}`);
            }
            // skip emitting the chain update event as we will manually do it
            await this.chain.putBlocks(blocks, true, true);
            for (const block of blocks) {
                const receipts = this.pendingReceipts?.get((0, util_1.bytesToHex)(block.hash()));
                if (receipts) {
                    await this.receiptsManager?.saveReceipts(block, receipts);
                    this.pendingReceipts?.delete((0, util_1.bytesToHex)(block.hash()));
                }
            }
            ...
            const td = await this.chain.getTd(vmHeadBlock.header.parentHash, vmHeadBlock.header.number - util_1.BIGINT_1);
     ...
     */
    async run(loop = true, runOnlybatched = false) {
        if (this.running || !this.started || this.config.shutdown)
            return 0;
        return this.runWithLock(async () => {
            try {
                ...
                this.config.logger.debug(`Running execution startHeadBlock=${startHeadBlock?.header.number} canonicalHead=${canonicalHead?.header.number} loop=${loop}`);
                let headBlock;
                let parentState;
                let errorBlock;
                ...
                while (this.started &&
                    ...
                    (0, util_1.equalsBytes)(startHeadBlock.hash(), canonicalHead.hash()) === false) {
                    let txCounter = 0;
                    headBlock = undefined;
                    parentState = undefined;
                    errorBlock = undefined;
                    this.vmPromise = blockchain
                        .iterator('vm', async (block, reorg) => {
                        ...
                        if (headBlock === undefined || reorg) {
                            ...
                            if (reorg) {
                                clearCache = true;
                                this.config.logger.info(`VM run: Chain reorged, setting new head to block number=${headBlock.header.number} clearCache=${clearCache}.`);
                            }
                            else {
                                const prevVMStateRoot = await this.vm.stateManager.getStateRoot();
                                clearCache = !(0, util_1.equalsBytes)(prevVMStateRoot, parentState);
                            }
                        }
                        else {
                        ...
                        try {
                            ...
                            if (hardfork !== this.hardfork) {
                                const wasPrePrague = !this.config.execCommon.gteHardfork(common_1.Hardfork.Prague);
                                const hash = (0, util_2.short)(block.hash());
                                this.config.superMsg(`Execution hardfork switch on block number=${number} hash=${hash} old=${this.hardfork} new=${hardfork}`);
                                this.hardfork = this.config.execCommon.setHardforkBy({
                                    blockNumber: number,
                                    td,
                                    timestamp,
                                });
                                const isPostPrague = this.config.execCommon.gteHardfork(common_1.Hardfork.Prague);
                            ...
                            if ((!this.config.execCommon.gteHardfork(common_1.Hardfork.Prague) &&
                                this.vm.stateManager instanceof statemanager_1.StatelessVerkleStateManager) ||
                                (this.config.execCommon.gteHardfork(common_1.Hardfork.Prague) &&
                                    this.vm.stateManager instanceof statemanager_1.DefaultStateManager)) {
                                throw Error(`Invalid vm stateManager type=${typeof this.vm.stateManager} for fork=${this.hardfork}`);
                            }
                            let skipBlockValidation = false;
                            if (this.config.execCommon.consensusType() === common_1.ConsensusType.ProofOfAuthority) {
                            ...
                            if (diffSec > this.MAX_TOLERATED_BLOCK_TIME) {
                                const msg = `Slow block execution for block num=${block.header.number} hash=${(0, util_1.bytesToHex)(block.hash())} txs=${block.transactions.length} gasUsed=${result.gasUsed} time=${diffSec}secs`;
                    ...
                    true)
                        ...
                        if (errorBlock !== undefined) {
                            ...
                            const hash = (0, util_2.short)(errorBlock.hash());
                            ...
                            if (`${error}`.toLowerCase().includes('does not contain state root') &&
                                number > util_1.BIGINT_1) {
                                ...
                                let backStepTo, backStepToHash, backStepToRoot, hasParentStateRoot = false;
                                if (headBlock !== undefined) {
                                    hasParentStateRoot = await this.vm.stateManager.hasStateRoot(headBlock.header.stateRoot);
                                    backStepTo = headBlock.header.number ?? util_1.BIGINT_0 - util_1.BIGINT_1;
                                ...
                                if (hasParentStateRoot === true && backStepToHash !== undefined) {
                                    this.config.logger.warn(`${errorMsg}, backStepping vmHead to number=${backStepTo} hash=${(0, util_2.short)(backStepToHash ?? 'na')} hasParentStateRoot=${(0, util_2.short)(backStepToRoot ?? 'na')}:\n${error}`);
                                    await this.vm.blockchain.setIteratorHead('vm', backStepToHash);
                                }
                                else {
                                    this.config.logger.error(`${errorMsg}, couldn't back step to vmHead number=${backStepTo} hash=${(0, util_2.short)(backStepToHash ?? 'na')} hasParentStateRoot=${hasParentStateRoot} backStepToRoot=${(0, util_2.short)(backStepToRoot ?? 'na')}:\n${error}`);
                                }
                            }
                            else {
                            ...
                            if (this.config.debugCode) {
                                await (0, debug_1.debugCodeReplayBlock)(this, errorBlock);
                            }
                            this.config.events.emit(types_1.Event.SYNC_EXECUTION_VM_ERROR, error);
                    ...
                    if (numExecuted !== null) {
                        ...
                        if (typeof numExecuted === 'number' && numExecuted > 0) {
                            ...
                            const firstHash = (0, util_2.short)(startHeadBlock.hash());
                            const lastNumber = endHeadBlock.header.number;
                            const lastHash = (0, util_2.short)(endHeadBlock.hash());
                            const baseFeeAdd = this.config.execCommon.gteHardfork(common_1.Hardfork.London) === true
                                ? `baseFee=${endHeadBlock.header.baseFeePerGas} `
                                : '';
                            const tdAdd = this.config.execCommon.gteHardfork(common_1.Hardfork.Paris) === true
                                ? ''
                                : `td=${this.chain.blocks.td} `;
                            (this.config.execCommon.gteHardfork(common_1.Hardfork.Paris) === true
                                ? this.config.logger.debug
                                : this.config.logger.info)(`Executed blocks count=${numExecuted} first=${firstNumber} hash=${firstHash} ${tdAdd}${baseFeeAdd}hardfork=${this.hardfork} last=${lastNumber} hash=${lastHash} txs=${txCounter}`);
                        }
                        else {
                            this.config.logger.debug(`No blocks executed past chain head hash=${(0, util_2.short)(endHeadBlock.hash())} number=${endHeadBlock.header.number}`);
                        }
    ...
    async start() {
        ...
        await this.stats.bind(this), this.STATS_INTERVAL);
        const { blockchain } = this.vm;
        ...
        const infoStr = `vmHead=${vmHeadBlock.header.number} canonicalHead=${canonicalHead.header.number} hardfork=${this.config.execCommon.hardfork()} execution=${this.config.execution}`;
        if (!this.config.execCommon.gteHardfork(common_1.Hardfork.Paris) &&
     ...
     */
    async executeBlocks(first, last, txHashes) {
        ...
        for (let blockNumber = first; blockNumber <= last; blockNumber++) {
            ...
            if (txHashes.length === 0) {
                ...
                const afterTS = Date.now();
                const diffSec = Math.round((afterTS - beforeTS) / 1000);
                const msg = `Executed block num=${blockNumber} hash=${(0, util_1.bytesToHex)(block.hash())} txs=${block.transactions.length} gasUsed=${res.gasUsed} time=${diffSec}secs`;
                if (diffSec <= this.MAX_TOLERATED_BLOCK_TIME) {
                ...
                for (const tx of block.transactions) {
                    ...
                    if (allTxs || txHashes.includes(txHash)) {
                        ...
                        this.config.logger.info(`Executed tx hash=${txHash} gasUsed=${res.totalGasSpent} from block num=${blockNumber}`);
                        count += 1;
                    }
                }
                if (count === 0) {
    ...
    stats() {
        if (this._statsVM instanceof statemanager_1.DefaultStateManager) {
            const sm = this._statsVM.stateManager;
            ...
            this.config.logger.info(`Account cache stats size=${stats.size} reads=${stats.reads} hits=${stats.hits} writes=${stats.writes}`);
            // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
            stats = !sm._storageCacheSettings.deactivate ? sm._storageCache.stats() : disactivatedStats;
            this.config.logger.info(`Storage cache stats size=${stats.size} reads=${stats.reads} hits=${stats.hits} writes=${stats.writes}`);
            // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
            stats = !sm._codeCacheSettings.deactivate ? sm._codeCache.stats() : disactivatedStats;
            this.config.logger.info(`Code cache stats size=${stats.size} reads=${stats.reads} hits=${stats.hits} writes=${stats.writes}`);
            const tStats = sm._trie.database().stats();
            this.config.logger.info(`Trie cache stats size=${tStats.size} reads=${tStats.cache.reads} hits=${tStats.cache.hits} ` +
