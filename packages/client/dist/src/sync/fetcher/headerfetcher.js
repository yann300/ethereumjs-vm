...
class HeaderFetcher extends blockfetcherbase_1.BlockFetcherBase {
    ...
    async store(headers) {
        try {
            ...
            this.debug(`Fetcher results stored in blockchain (headers num=${headers.length} first=${headers[0]?.number} last=${headers[headers.length - 1]?.number})`);
            this.config.events.emit(types_1.Event.SYNC_FETCHED_HEADERS, headers.slice(0, num));
        }
        catch (e) {
            this.debug(`Error storing fetcher results in blockchain (headers num=${headers.length} first=${headers[0]?.number} last=${headers[headers.length - 1]?.number}): ${e}`);
            throw e;
        }
    }
    /**
