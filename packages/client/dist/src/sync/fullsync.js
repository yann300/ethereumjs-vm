...
class FullSynchronizer extends sync_1.Synchronizer {
    ...
    get fetcher() {
        if (this._fetcher !== null && !(this._fetcher instanceof fetcher_1.BlockFetcher)) {
    ...
    async sync() {
        ...
        const syncEvent = new Promise((resolve) => {
            ...
            this.config.events.once(types_1.Event.SYNC_SYNCHRONIZED, (height) => {
    ...
    async open() {
        ...
        this.config.events.on(types_1.Event.SYNC_FETCHED_BLOCKS, this.processBlocks);
        this.config.events.on(types_1.Event.SYNC_EXECUTION_VM_ERROR, this.stop);
        if (this.config.execution) {
            this.config.events.on(types_1.Event.CHAIN_UPDATED, this.runExecution);
        }
        ...
        this.config.logger.info(`Latest local block number=${Number(number)} td=${td} hash=${(0, util_2.short)(hash)} hardfork=${this.config.chainCommon.hardfork()}`);
    }
    /**
    ...
    async best() {
        ...
        for (const peer of peers) {
            if (peer.eth?.status !== undefined) {
                ...
                if ((!best && td >= this.chain.blocks.td) ||
    ...
    checkTxPoolState() {
        ...
        const target = this.config.syncTargetHeight - BigInt(this.txPool.BLOCKS_BEFORE_TARGET_HEIGHT_ACTIVATION);
    ...
    async syncWithPeer(peer) {
        ...
        const first = this.chain.blocks.height >= BigInt(this.config.safeReorgDistance)
            ? this.chain.blocks.height - BigInt(this.config.safeReorgDistance) + util_1.BIGINT_1
            : util_1.BIGINT_1;
        const count = height - first + util_1.BIGINT_1;
        if (count < util_1.BIGINT_0)
            return false;
        if (!this.fetcher || this.fetcher.syncErrored) {
            this.fetcher = new fetcher_1.BlockFetcher({
                config: this.config,
                pool: this.pool,
                chain: this.chain,
    ...
    async processBlocks(blocks) {
        ...
        const hash = (0, util_2.short)(blocks[0].hash());
        ...
        if (nextHFBlockNum !== null) {
            ...
            if (this.config.chainCommon.hardfork() === common_1.Hardfork.MergeForkIdTransition &&
                this.config.chainCommon.gteHardfork(common_1.Hardfork.Paris) === false) {
                const mergeTTD = this.config.chainCommon.hardforkTTD(common_1.Hardfork.Paris);
        ...
        this.config.logger.info(`Imported blocks count=${blocks.length} first=${first} last=${last} hash=${hash} ${baseFeeAdd}hardfork=${this.config.chainCommon.hardfork()} peers=${this.pool.size}`, { attentionHF });
     ...
     */
    addToKnownByPeer(blockHash, peer) {
        const knownBlocks = this.newBlocksKnownByPeer.get(peer.id) ?? [];
        if (knownBlocks.find((knownBlock) => (0, util_1.equalsBytes)(knownBlock.hash, blockHash))) {
            return true;
        }
     ...
     * @param peers
     */
    async sendNewBlock(block, peers) {
        for (const peer of peers) {
    ...
    async handleNewBlock(block, peer) {
        ...
        if (block.header.number > this.chain.headers.height + util_1.BIGINT_1) {
        ...
        try {
            ...
            this.config.logger.debug(`Error processing new block from peer ${peer ? `id=${peer.id.slice(0, 8)}` : '(no peer)'} hash=${(0, util_2.short)(block.hash())}`);
        ...
        if (latestBlockHash !== undefined &&
            (0, util_1.equalsBytes)(latestBlockHash, block.header.parentHash) === true) {
     ...
     */
    handleNewBlockHashes(data) {
        if (!data.length || !this.fetcher || this.fetcher.syncErrored)
            return;
        let min = BigInt(-1);
        let newSyncHeight;
        ...
        for (const value of data) {
            ...
            // Check if new sync target height can be set
            if (newSyncHeight && blockNumber <= newSyncHeight[1])
                continue;
            if (typeof this.config.syncTargetHeight === 'bigint' &&
                this.config.syncTargetHeight !== util_1.BIGINT_0 &&
                blockNumber <= this.config.syncTargetHeight)
                continue;
            newSyncHeight = value;
        ...
        this.config.syncTargetHeight = height;
        this.config.logger.info(`New sync target height=${height} hash=${(0, util_2.short)(hash)}`);
    ...
    async runExecution() {
        ...
        const shouldRunOnlyBatched = typeof this.config.syncTargetHeight === 'bigint' &&
            this.config.syncTargetHeight !== util_1.BIGINT_0 &&
    ...
    async stop() {
        this.config.events.removeListener(types_1.Event.SYNC_FETCHED_BLOCKS, this.processBlocks);
        this.config.events.removeListener(types_1.Event.SYNC_EXECUTION_VM_ERROR, this.stop);
        if (this.config.execution) {
            this.config.events.removeListener(types_1.Event.CHAIN_UPDATED, this.runExecution);
        }
        return super.stop();
    }
    /**
