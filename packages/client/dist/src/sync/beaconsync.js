...
exports.BeaconSynchronizer = void 0;
 ...
 */
class BeaconSynchronizer extends sync_1.Synchronizer {
    constructor(options) {
        super(options);
        this.running = false;
    ...
    get fetcher() {
        if (this._fetcher !== null && !(this._fetcher instanceof fetcher_1.ReverseBlockFetcher)) {
    ...
    async open() {
        ...
        this.config.events.on(types_1.Event.SYNC_FETCHED_BLOCKS, this.processSkeletonBlocks);
        ...
        this.config.logger.info(`Latest local block number=${Number(number)} td=${td} hash=${(0, util_1.bytesToHex)(hash)} hardfork=${this.config.chainCommon.hardfork()}`);
        const subchain = this.skeleton.bounds();
        if (subchain !== undefined) {
            const { head, tail, next } = subchain;
            this.config.superMsg(`Resuming beacon sync tail=${tail} head=${head} next=${(0, util_2.short)(next)}`);
    ...
    async reorged(block) {
        ...
        await this.stop();
        this.config.logger.debug(`Beacon sync reorged, new head number=${block.header.number} hash=${(0, util_2.short)(block.header.hash())}`);
        void this.start();
    }
    /**
     * Returns true if the block successfully extends the chain.
     */
    async extendChain(block) {
        if (!this.opened)
            return false;
    ...
    async syncWithPeer(peer) {
        ...
        const first = tail - util_1.BIGINT_1;
        let count;
        if (first <= this.chain.blocks.height) {
            ...
            count = tail - this.chain.blocks.height - util_1.BIGINT_1;
        }
        // Do not try syncing blocks on/pre genesis
        if (count > first) {
            count = first;
        }
        if (count > util_1.BIGINT_0 && (this.fetcher === null || this.fetcher.syncErrored !== undefined)) {
            this.config.logger.debug(`syncWithPeer - new ReverseBlockFetcher peer=${peer?.id} subChainTail=${tail} first=${first} count=${count} chainHeight=${this.chain.blocks.height} ${this.fetcher === null
                ? ''
                : 'previous fetcher errored=' + this.fetcher.syncErrored?.message}`);
            this.fetcher = new fetcher_1.ReverseBlockFetcher({
                config: this.config,
                pool: this.pool,
                chain: this.chain,
    ...
    async processSkeletonBlocks(blocks) {
        if (blocks.length === 0) {
        ...
        this.config.logger.debug(`Imported skeleton blocks count=${blocks.length} first=${first} last=${last} hash=${hash} peers=${this.pool.size}`);
    }
    /**
    ...
    async runExecution() {
        ...
        const shouldRunOnlyBatched = !(this.skeleton.bounds() !== undefined &&
            this.chain.blocks.height > this.skeleton.bounds().head - BigInt(50));
        if (!shouldRunOnlyBatched || this.chain.blocks.height % BigInt(50) === util_1.BIGINT_0) {
    ...
    async close() {
        if (!this.opened)
            return;
        this.config.events.removeListener(types_1.Event.SYNC_FETCHED_BLOCKS, this.processSkeletonBlocks);
        if (this.config.execution) {
...
exports.BeaconSynchronizer = BeaconSynchronizer;
//# sourceMappingURL=beaconsync.js.map