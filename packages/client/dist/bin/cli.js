...
const level_2 = require("../src/execution/level");
const logging_1 = require("../src/logging");
const types_1 = require("../src/types");
const util_2 = require("../src/util");
const startRpc_1 = require("./startRpc");
const networks = Object.entries(common_1.Common.getInitializedChains().names);
let logger;
const args = yargs
    .default((0, helpers_1.hideBin)(process.argv))
    .parserConfiguration({
    'dot-notation': false,
})
    .option('network', {
    ...
    choices: Object.values(config_1.SyncMode),
    ...
    default: `${(0, os_1.homedir)()}/Library/Ethereum/ethereumjs`,
})
    .option('customChain', {
    describe: 'Path to custom chain parameters json file (@ethereumjs/common format)',
    coerce: (arg) => (arg ? path.resolve(arg) : undefined),
    ...
    string: true,
    coerce: (customGenesisStateRoot) => (0, util_1.hexToBytes)(customGenesisStateRoot),
})
    .option('gethGenesis', {
    ...
    .option('bootnodes', {
    describe: 'Comma-separated list of network bootnodes (format: "enode://<id>@<host:port>,enode://..." ("[?discport=<port>]" not supported) or path to a bootnode.txt file',
    array: true,
    coerce: (arr) => arr.filter((el) => el !== undefined).map((el) => el.toString()),
})
    .option('port', {
    ...
    .option('logFile', {
    describe: 'File to save log file (default logs to `$dataDir/ethereumjs.log`, pass false to disable)',
    default: true,
})
    .option('logLevelFile', {
    ...
    .option('rpcDebug', {
    describe: 'Additionally log truncated RPC calls filtered by name (prefix), e.g.: "eth,engine_getPayload" (use "all" for all methods). Truncated by default, add verbosity using "rpcDebugVerbose"',
    default: '',
    string: true,
})
    .option('rpcDebugVerbose', {
    describe: 'Additionally log complete RPC calls filtered by name (prefix), e.g.: "eth,engine_getPayload" (use "all" for all methods).',
    default: '',
    string: true,
})
    .option('rpcCors', {
    ...
    number: true,
    default: config_1.Config.MAXPERREQUEST_DEFAULT,
})
    .option('maxFetcherJobs', {
    describe: 'Max tasks or jobs to be created for a fetcher at a time',
    number: true,
    default: config_1.Config.MAXFETCHERJOBS_DEFAULT,
})
    .option('minPeers', {
    ...
    describe: 'Address for mining rewards (etherbase). If not provided, defaults to the primary account',
    string: true,
    coerce: (coinbase) => util_1.Address.fromString(coinbase),
})
    .option('saveReceipts', {
    describe: 'Save tx receipts and logs in the meta db (warning: may use a large amount of storage). With `--rpc` allows querying via eth_getLogs (max 10000 logs per request) and eth_getTransactionReceipt (within `--txLookupLimit`)',
    boolean: true,
    default: true,
})
    .option('snap', {
    ...
    describe: 'Enable/Disable storage trie prefixes (specify `false` for backward compatibility with previous states synced without prefixes)',
    boolean: true,
    default: true,
    ...
    describe: 'Use string values in the trie DB. This is old behavior, new behavior uses Uint8Arrays in the DB (more performant)',
    boolean: true,
    default: false,
    ...
    describe: 'Number of recent blocks to maintain transactions index for (default = about one year, 0 = entire chain)',
    number: true,
    default: 2350000,
})
    .option('startBlock', {
    describe: 'Block number to start syncing from. Must be lower than the local chain tip. Note: this is destructive and removes blocks from the blockchain, please back up your datadir before using.',
    number: true,
})
    .option('isSingleNode', {
    describe: 'To run client in single node configuration without need to discover the sync height from peer. Particularly useful in test configurations. This flag is automically activated in the "dev" mode',
    boolean: true,
})
    ...
    describe: 'Number of unexecuted blocks (including ancestors) that can be blockingly executed in engine`s new payload (if required and possible) to determine the validity of the block',
    number: true,
})
    .option('skipEngineExec', {
    describe: 'Skip executing blocks in new payload calls in engine, alias for --engineNewpayloadMaxExecute=0 and overrides any engineNewpayloadMaxExecute if also provided',
    boolean: true,
})
    .option('useJsCrypto', {
    ...
    if (argv.rpc === true && usedPorts.has(argv.rpcPort))
        collision = true;
    if (argv.rpcEngine === true && usedPorts.has(argv.rpcEnginePort))
        collision = true;
    if (collision)
        throw new Error('cannot reuse ports between RPC instances');
    return true;
})
    .parseSync();
/**
...
function initDBs(config) {
    // Chain DB
    const chainDataDir = config.getDataDirectory(config_1.DataDirectory.Chain);
    (0, fs_extra_1.ensureDirSync)(chainDataDir);
    const chainDB = new level_1.Level(chainDataDir);
    // State DB
    const stateDataDir = config.getDataDirectory(config_1.DataDirectory.State);
    (0, fs_extra_1.ensureDirSync)(stateDataDir);
    const stateDB = new level_1.Level(stateDataDir);
    // Meta DB (receipts, logs, indexes, skeleton chain)
    const metaDataDir = config.getDataDirectory(config_1.DataDirectory.Meta);
    (0, fs_extra_1.ensureDirSync)(metaDataDir);
...
async function executeBlocks(client) {
    ...
    try {
        ...
        client.config.logger.error('Wrong input format for block execution, allowed format types: 5, 5-10, 5[0xba4b5fd92a26badad3cad22eb6f7c7e745053739b5f5d1e8a3afb00f8fb2a280,[TX_HASH_2],...], 5[*] (all txs in verbose mode)');
        process.exit();
    }
    const { execution } = client.services.find((s) => s.name === 'eth');
    if (execution === undefined)
        throw new Error('executeBlocks requires execution');
...
async function startClient(config, genesisMeta = {}) {
    ...
    if (genesisMeta.genesisState !== undefined || genesisMeta.genesisStateRoot !== undefined) {
        const validateConsensus = config.chainCommon.consensusAlgorithm() === common_1.ConsensusAlgorithm.Clique;
        blockchain = await blockchain_1.Blockchain.create({
            db: new level_2.LevelDB(dbs.chainDB),
            ...genesisMeta,
    ...
    const client = await client_1.EthereumClient.create({
        config,
        blockchain,
        ...genesisMeta,
        ...dbs,
    ...
    if (args.loadBlocksFromRlp !== undefined) {
        ...
        const blockRlp = (0, fs_extra_1.readFileSync)(args.loadBlocksFromRlp);
        const blocks = [];
        let buf = rlp_1.RLP.decode(blockRlp, true);
        while (buf.data?.length > 0 || buf.remainder?.length > 0) {
            try {
                const block = block_1.Block.fromValuesArray(buf.data, {
                    common: config.chainCommon,
                    setHardfork: true,
                });
                blocks.push(block);
                buf = rlp_1.RLP.decode(buf.remainder, true);
                config.logger.info(`Preloading block hash=0x${(0, util_1.short)((0, util_1.bytesToHex)(block.header.hash()))} number=${block.header.number}`);
            }
            catch (err) {
    ...
    if (args.loadBlocksFromRlp !== undefined && client.chain.opened) {
        const service = client.service('eth');
 ...
 */
async function setupDevnet(prefundAddress) {
    const addr = prefundAddress.toString().slice(2);
    const consensusConfig = args.dev === 'pow'
        ? { ethash: true }
        : {
            clique: {
                period: 10,
    ...
    const extraData = args.dev === 'pow' ? '0x' + '0'.repeat(32) : '0x' + '0'.repeat(64) + addr + '0'.repeat(130);
    const chainData = {
        ...defaultChainData,
        extraData,
        alloc: { [addr]: { balance: '0x10000000000000000000' } },
    };
    const common = common_1.Common.fromGethGenesis(chainData, { chain: 'devnet', hardfork: common_1.Hardfork.London });
    const customGenesisState = (0, util_1.parseGethGenesisState)(chainData);
...
async function inputAccounts() {
    ...
    rl.input.on('keypress', function () {
        ...
        readline_1.default.moveCursor(rl.output, -len, 0);
        // clear everything to the right of the cursor:
        readline_1.default.clearLine(rl.output, 1);
    ...
    try {
        ...
        const isFile = (0, fs_1.existsSync)(path.resolve(addresses[0]));
        if (!isFile) {
            for (const addressString of addresses) {
                const address = util_1.Address.fromString(addressString);
                const inputKey = await question(`Please enter the 0x-prefixed private key to unlock ${address}:\n`);
                rl.history = rl.history.slice(1);
                const privKey = (0, util_1.toBytes)(inputKey);
                const derivedAddress = util_1.Address.fromPrivateKey(privKey);
                if (address.equals(derivedAddress)) {
                    accounts.push([address, privKey]);
                }
                else {
                    console.error(`Private key does not match for ${address} (address derived: ${derivedAddress})`);
                    process.exit();
                }
            }
        }
        else {
            const acc = (0, fs_extra_1.readFileSync)(path.resolve(args.unlock), 'utf-8').replace(/(\r\n|\n|\r)/gm, '');
            const privKey = (0, util_1.hexToBytes)('0x' + acc); // See docs: acc has to be non-zero prefixed in the file
            const derivedAddress = util_1.Address.fromPrivateKey(privKey);
...
function generateAccount() {
    const privKey = (0, util_1.randomBytes)(32);
    const address = util_1.Address.fromPrivateKey(privKey);
    console.log('='.repeat(50));
    console.log('Account generated for mining blocks:');
    console.log(`Address: ${address}`);
    console.log(`Private key: ${(0, util_1.bytesToHex)(privKey)}`);
...
async function run() {
    ...
    const chain = args.networkId ?? args.network ?? common_1.Chain.Mainnet;
    const cryptoFunctions = {};
    const kzg = await (0, kzg_wasm_1.createKZG)();
    (0, util_1.initKZG)(kzg);
    // Initialize WASM crypto if JS crypto is not specified
    if (args.useJsCrypto === false) {
        await (0, wasm_crypto_1.waitReady)();
        cryptoFunctions.keccak256 = wasm_crypto_1.keccak256;
        cryptoFunctions.ecrecover = (msgHash, v, r, s, chainID) => (0, wasm_crypto_1.secp256k1Expand)((0, wasm_crypto_1.secp256k1Recover)(msgHash, (0, util_1.concatBytes)((0, util_1.setLengthLeft)(r, 32), (0, util_1.setLengthLeft)(s, 32)), Number((0, util_1.calculateSigRecovery)(v, chainID)))).slice(1);
        cryptoFunctions.sha256 = wasm_crypto_1.sha256;
        cryptoFunctions.ecsign = (msg, pk, chainId) => {
            if (msg.length < 32) {
            ...
            const buf = (0, wasm_crypto_1.secp256k1Sign)(msg, pk);
            const r = buf.slice(0, 32);
            const s = buf.slice(32, 64);
            const v = chainId === undefined
                ? BigInt(buf[64] + 27)
                : BigInt(buf[64] + 35) + BigInt(chainId) * util_1.BIGINT_2;
            return { r, s, v };
        };
        cryptoFunctions.ecdsaSign = (hash, pk) => {
            const sig = (0, wasm_crypto_1.secp256k1Sign)(hash, pk);
            return {
                signature: sig.slice(0, 64),
                recid: sig[64],
            };
        };
        cryptoFunctions.ecdsaRecover = (sig, recId, hash) => {
            return (0, wasm_crypto_1.secp256k1Recover)(hash, sig, recId);
        };
    }
    else {
        cryptoFunctions.keccak256 = keccak_1.keccak256;
        cryptoFunctions.ecrecover = util_1.ecrecover;
        cryptoFunctions.sha256 = sha256_1.sha256;
        cryptoFunctions.ecsign = util_1.ecsign;
        cryptoFunctions.ecdsaSign = secp256k1_compat_1.ecdsaSign;
        cryptoFunctions.ecdsaRecover = secp256k1_compat_1.ecdsaRecover;
    }
    ...
    let customGenesisState;
    let common = new common_1.Common({ chain, hardfork: common_1.Hardfork.Chainstart, customCrypto: cryptoFunctions });
    if (args.dev === true || typeof args.dev === 'string') {
        ...
        if (accounts.length === 0) {
            ...
            (0, fs_extra_1.removeSync)(`${args.dataDir}/devnet`);
            // Create new account
    ...
    if (typeof args.customChain === 'string') {
        try {
            const customChainParams = JSON.parse((0, fs_extra_1.readFileSync)(args.customChain, 'utf-8'));
            customGenesisState = JSON.parse((0, fs_extra_1.readFileSync)(args.customGenesisState, 'utf-8'));
        ...
        const genesisFile = JSON.parse((0, fs_extra_1.readFileSync)(args.gethGenesis, 'utf-8'));
        const chainName = path.parse(args.gethGenesis).base.split('.')[0];
        common = common_1.Common.fromGethGenesis(genesisFile, {
            chain: chainName,
            mergeForkIdPostMerge: args.mergeForkIdPostMerge,
        });
        //@ts-ignore
        common.customCrypto = cryptoFunctions;
        customGenesisState = (0, util_1.parseGethGenesisState)(genesisFile);
    }
    if (args.mine === true && accounts.length === 0) {
        console.error('Please provide an account to mine blocks with `--unlock [address]` or use `--dev` to generate');
        process.exit();
    }
    ...
    (0, fs_extra_1.ensureDirSync)(configDirectory);
    ...
    logger = (0, logging_1.getLogger)(args);
    let bootnodes;
    if (args.bootnodes !== undefined) {
        // File path passed, read bootnodes from disk
        if (Array.isArray(args.bootnodes) &&
            args.bootnodes.length === 1 &&
            args.bootnodes[0].includes('.txt')) {
            const file = (0, fs_extra_1.readFileSync)(args.bootnodes[0], 'utf-8');
            ...
            bootnodes = (0, util_2.parseMultiaddrs)(nodeURLs);
            logger.info(`Reading bootnodes file=${args.bootnodes[0]} num=${nodeURLs.length}`);
        }
        else {
            bootnodes = (0, util_2.parseMultiaddrs)(args.bootnodes);
        }
    }
    const multiaddrs = args.multiaddrs !== undefined ? (0, util_2.parseMultiaddrs)(args.multiaddrs) : undefined;
    ...
    config.events.on(types_1.Event.SERVER_LISTENING, (details) => {
        const networkDir = config.getNetworkDirectory();
        // Write the transport into a file
        try {
            (0, fs_1.writeFileSync)(`${networkDir}/${details.transport}`, details.url);
        }
        catch (e) {
            // In case dir is not really setup, mostly to take care of mockserver in test
            config.logger.error(`Error writing listener details to disk: ${e.message}`);
        }
    });
    ...
    const clientStartPromise = startClient(config, {
        ...
        const servers = args.rpc === true || args.rpcEngine === true || args.ws === true
            ? (0, startRpc_1.startRPCServers)(client, args)
            : [];
        if (client.config.chainCommon.gteHardfork(common_1.Hardfork.Paris) === true &&
