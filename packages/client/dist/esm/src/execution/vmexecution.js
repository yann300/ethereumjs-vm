...
import { DBSaveLookups, DBSetBlockOrHeader, DBSetHashToNumber, DBSetTD, } from '@ethereumjs/blockchain';
import { ConsensusType, Hardfork } from '@ethereumjs/common';
import { MCLBLS, RustBN254 } from '@ethereumjs/evm';
import { getGenesis } from '@ethereumjs/genesis';
import { createMPT } from '@ethereumjs/mpt';
import { CacheType, Caches, MerkleStateManager, StatefulVerkleStateManager, StatelessVerkleStateManager, } from '@ethereumjs/statemanager';
import { BIGINT_0, BIGINT_1, EthereumJSErrorWithoutCode, Lock, ValueEncoding, bytesToHex, equalsBytes, hexToBytes, } from '@ethereumjs/util';
...
export class VMExecution extends Execution {
    ...
    async open() {
        return this.runWithLock(async () => {
            ...
            if (typeof blockchain.getIteratorHead !== 'function') {
                throw EthereumJSErrorWithoutCode('cannot get iterator head: blockchain has no getIteratorHead function');
            }
            ...
            if (typeof blockchain.getTotalDifficulty !== 'function') {
                throw EthereumJSErrorWithoutCode('cannot get iterator head: blockchain has no getTotalDifficulty function');
            }
            ...
            if (number === BIGINT_0) {
                const genesisState = this.chain['_customGenesisState'] ?? getGenesis(Number(blockchain.common.chainId()));
                if (!genesisState &&
        ...
        await this.runWithLock(async () => {
            try {
                ...
                if (receipts === undefined) {
                    ...
                    const needsStatelessExecution = typeof this.vm.stateManager.initVerkleExecutionWitness === 'function';
                    ...
                    const skipHeaderValidation = needsStatelessExecution && this.chain.headers.height < block.header.number - BIGINT_1;
    ...
    async setHead(blocks, { finalizedBlock, safeBlock } = {}) {
        ...
        return this.runWithLock(async () => {
            ...
            if (!(await this.vm.stateManager.hasStateRoot(vmHeadBlock.header.stateRoot))) {
                ...
                throw Error(`vmHeadBlock's stateRoot not found number=${vmHeadBlock.header.number} root=${short(vmHeadBlock.header.stateRoot)}`);
            }
    ...
    async jumpVmHead(jumpToHash, jumpToNumber) {
        return this.runWithLock(async () => {
            // check if the block is canonical in chain
            this.config.logger?.warn(`Setting execution head to hash=${short(jumpToHash)} number=${jumpToNumber}`);
    ...
    async run(loop = true, runOnlyBatched = false) {
        if (this.running || !this.started || !this.config.execution || this.config.shutdown)
            return 0;
        return this.runWithLock(async () => {
            try {
                ...
                while (this.started &&
                    ...
                    this.vmPromise = this.chain.blockchain
                        ...
                        try {
                            ...
                            if (diffSec > this.MAX_TOLERATED_BLOCK_TIME) {
                                const msg = `Slow block execution for block num=${block.header.number} hash=${bytesToHex(block.hash())} txs=${block.transactions.length} gasUsed=${result.gasUsed} time=${diffSec}secs`;
                        ...
                        if (errorBlock !== undefined) {
                            ...
                            if (`${error}`.toLowerCase().includes('does not contain state root') &&
                                ...
                                if (hasParentStateRoot === true && backStepToHash !== undefined) {
                                    this.config.logger?.warn(`${errorMsg}, backStepping vmHead to number=${backStepTo} hash=${short(backStepToHash ?? 'na')} hasParentStateRoot=${short(backStepToRoot ?? 'na')}:\n${error}`);
                                    await this.chain.blockchain.setIteratorHead('vm', backStepToHash);
                                }
                                else {
                                    this.config.logger?.error(`${errorMsg}, couldn't back step to vmHead number=${backStepTo} hash=${short(backStepToHash ?? 'na')} hasParentStateRoot=${hasParentStateRoot} backStepToRoot=${short(backStepToRoot ?? 'na')}:\n${error}`);
                                }
                            }
                            else {
                                this.chainStatus = {
                                    ...
                                    status: this.config.ignoreStatelessInvalidExecs === true
                    ...
                    if (numExecuted !== null) {
                        ...
                        if (typeof numExecuted === 'number' && numExecuted > 0) {
                            ...
                            this.config.logger?.debug(`No blocks executed past chain head hash=${short(endHeadBlock.hash())} number=${endHeadBlock.header.number}`);
                        }
    ...
    async start() {
        ...
        if ((!this.config.execCommon.gteHardfork(Hardfork.Paris) || this.config.startExecution) &&
    ...
    async executeBlocks(first, last, txHashes) {
        ...
        if (this.config.execCommon.hardforkGteHardfork(startExecutionHardfork, Hardfork.Verkle) &&
        ...
        for (let blockNumber = first; blockNumber <= last; blockNumber++) {
            ...
            if (txHashes.length === 0) {
                ...
                const afterTS = Date.now();
                const diffSec = Math.round((afterTS - beforeTS) / 1000);
                const msg = `Executed block num=${blockNumber} hash=${bytesToHex(block.hash())} txs=${block.transactions.length} gasUsed=${res.gasUsed} time=${diffSec}secs`;
                if (diffSec <= this.MAX_TOLERATED_BLOCK_TIME) {
                    this.config.logger?.info(msg);
                }
                else {
