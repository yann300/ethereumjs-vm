...
import { BIGINT_0, BIGINT_1, BIGINT_2, BIGINT_256, bigIntToBytes, bigIntToHex, bytesToBigInt, bytesToHex, compareBytes, setLengthLeft, short, } from '@ethereumjs/util';
...
export class StorageFetcher extends Fetcher {
    ...
    constructor(options) {
        ...
        if (this.storageRequests.length > 0) {
            ...
            this.DEBUG &&
                this.debug(`Storage fetcher instantiated with ${fullJob.task.storageRequests.length} accounts requested and root=${short(this.root)} origin=${short(origin)} limit=${short(limit)} destroyWhenDone=${this.destroyWhenDone}`);
        }
    ...
    async verifyRangeProof(stateRoot, origin, { slots, proof }) {
        try {
            this.DEBUG &&
                this.debug(`verifyRangeProof slots:${slots.length} first=${short(slots[0].hash)} last=${short(slots[slots.length - 1].hash)}`);
            const keys = slots.map((slot) => slot.hash);
            const values = slots.map((slot) => slot.body);
            return await verifyMerkleRangeProof(stateRoot, origin, keys[keys.length - 1], keys, values, proof ?? null, this.config.chainCommon?.customCrypto?.keccak256);
        }
        catch (err) {
            this.DEBUG && this.debug(`verifyRangeProof failure: ${err.stack}`);
    ...
    async request(job) {
        ...
        this.DEBUG &&
            this.debug(`requested account hashes: ${task.storageRequests.map((req) => bytesToHex(req.accountHash))}`);
        ...
        // fragmented request
        if (task.multi === false) {
            const highestKnownHash = this.accountToHighestKnownHash.get(bytesToHex(task.storageRequests[0].accountHash));
        ...
        if (rangeResult.slots.length === 0) {
            ...
            if (rangeResult.proof.length > 0) {
                try {
                    const isMissingRightRange = await verifyMerkleRangeProof(task.storageRequests[0].storageRoot, origin, null, [], [], rangeResult.proof);
        ...
        try {
            ...
            for (let i = 0; i < rangeResult.slots.length; i++) {
                ...
                if (proof === undefined || proof.length === 0) {
                    ...
                    await verifyMerkleRangeProof(root, null, null, accountSlots.map((s) => s.hash), accountSlots.map((s) => s.body), null, this.config.chainCommon?.customCrypto?.keccak256);
                    if (proof?.length === 0)
                    ...
                    if (task.multi === false) {
                        if (!hasRightElement) {
                            ...
                            if (this.isMissingRightRange(limit, rangeResult)) {
                                this.DEBUG &&
                                    this.debug(`Peer ${peerInfo} returned missing right range Slot=${bytesToHex(rangeResult.slots[0][rangeResult.slots.length - 1].hash)} limit=${bytesToHex(limit)}`);
                                completed = false;
                            }
                            else {
                                completed = true;
                    ...
                    if (hasRightElement) {
                        this.DEBUG &&
                            this.debug(`Account fragmented at ${bytesToHex(highestReceivedhash)} as part of multiaccount fetch`);
     ...
     */
    tasks(first = BIGINT_0, count = TOTAL_RANGE_END, maxTasks = this.config.maxFetcherJobs) {
        const tasks = [];
    ...
    nextTasks() {
        ...
        try {
            if (this.in.length === 0) {
                ...
                this.DEBUG &&
                    this.debug(`Fetcher pending with ${fullJob.task.storageRequests.length} accounts requested and origin=${short(origin)} limit=${short(limit)}`);
            }
        }
        catch (err) {
            this.DEBUG && this.debug(err);
