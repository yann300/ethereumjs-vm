...
import { BIGINT_0, BIGINT_1, BIGINT_2EXP256, BIGINT_100, KECCAK256_NULL, KECCAK256_RLP, accountBodyToRLP, bigIntToBytes, bytesToBigInt, bytesToHex, compareBytes, equalsBytes, formatBigDecimal, setLengthLeft, } from '@ethereumjs/util';
...
export class AccountFetcher extends Fetcher {
    ...
    constructor(options) {
        ...
        this.DEBUG &&
            this.debug(`Account fetcher instantiated root=${short(this.root)} origin=${short(origin)} limit=${short(limit)} destroyWhenDone=${this.destroyWhenDone}`);
    }
    async blockingFetch() {
        ...
        try {
            ...
            const storageFetch = !this.fetcherDoneFlags.storageFetcher.done
                ? this.storageFetcher.blockingFetch().then(() => this.snapFetchersCompleted(StorageFetcher), () => {
                    throw Error('Snap fetcher failed to exit');
                })
                : null;
            const codeFetch = !this.fetcherDoneFlags.byteCodeFetcher.done
                ? this.byteCodeFetcher.blockingFetch().then(() => this.snapFetchersCompleted(ByteCodeFetcher), () => {
                    throw Error('Snap fetcher failed to exit');
                })
                : null;
    ...
    snapFetchersCompleted(fetcherType, root) {
        ...
        switch (fetcherType) {
            case AccountFetcher:
                ...
                if (fetcherDoneFlags.accountFetcher.first !== BIGINT_2EXP256) {
                    const fetcherProgress = formatBigDecimal(fetcherDoneFlags.accountFetcher.first * BIGINT_100, BIGINT_2EXP256, BIGINT_100);
            ...
            case StorageFetcher:
                ...
                if (fetcherDoneFlags.storageFetcher.first !== fetcherDoneFlags.storageFetcher.count) {
                    const reqsDone = formatBigDecimal(fetcherDoneFlags.storageFetcher.first * BIGINT_100, fetcherDoneFlags.storageFetcher.count, BIGINT_100);
            ...
            case ByteCodeFetcher:
                ...
                if (fetcherDoneFlags.byteCodeFetcher.first !== fetcherDoneFlags.byteCodeFetcher.count) {
                    const reqsDone = formatBigDecimal(fetcherDoneFlags.byteCodeFetcher.first * BIGINT_100, fetcherDoneFlags.byteCodeFetcher.count, BIGINT_100);
        ...
        this.config.superMsg(`snapFetchersCompletion root=${short(this.root)} accountsRoot=${short(fetcherDoneFlags.stateRoot ?? 'na')} done=${this.fetcherDoneFlags.done} accountsDone=${accountFetcher.done} storageDone=${storageFetcher.done} byteCodesDone=${byteCodeFetcher.done} trieNodesDone=${trieNodeFetcher.done}`);
    ...
    async verifyRangeProof(stateRoot, origin, { accounts, proof }) {
        this.DEBUG &&
            this.debug(`verifyRangeProof accounts:${accounts.length} first=${bytesToHex(accounts[0].hash)} last=${short(accounts[accounts.length - 1].hash)}`);
        ...
        const values = accounts.map((acc) => accountBodyToRLP(acc.body));
        // convert the request to the right values
        return verifyMerkleRangeProof(stateRoot, origin, keys[keys.length - 1], keys, values, proof, this.config.chainCommon?.customCrypto?.keccak256);
    }
    getOrigin(job) {
        const { task, partialResult } = job;
        const { first } = task;
        // Snap protocol will automatically pad it with 32 bytes left, so we don't need to worry
        const origin = partialResult !== undefined
    ...
    async request(job) {
        ...
        if (rangeResult.accounts.length === 0 ||
            ...
            if (rangeResult.proof.length > 0) {
                try {
                    const isMissingRightRange = await verifyMerkleRangeProof(this.root, origin, null, [], [], rangeResult.proof);
        ...
        try {
            ...
            if (isMissingRightRange && this.isMissingRightRange(limit, rangeResult)) {
                this.DEBUG &&
                    this.debug(`Peer ${peerInfo} returned missing right range account=${bytesToHex(rangeResult.accounts[rangeResult.accounts.length - 1].hash)} limit=${bytesToHex(limit)}`);
                completed = false;
            }
            else {
                completed = true;
    ...
    process(job, result) {
        ...
        if (this.highestKnownHash) {
            if (highestReceivedhash !== undefined &&
    ...
    tasks(first = this.first, count = this.count, maxTasks = this.config.maxFetcherJobs) {
        ...
        debugStr += ` limit=${short(setLengthLeft(bigIntToBytes(startedWith + pushedCount - BIGINT_1), 32))}`;
