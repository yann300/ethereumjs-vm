...
export class FullSynchronizer extends Synchronizer {
    ...
    async sync() {
        ...
        const syncEvent = new Promise((resolve) => {
            ...
            this.config.events.once(Event.SYNC_SYNCHRONIZED, (chainHeight) => {
    ...
    async open() {
        ...
        this.config.logger?.info(`Latest local block number=${Number(number)} td=${td} hash=${short(hash)} hardfork=${this.config.chainCommon.hardfork()}`);
    }
    /**
    ...
    async best() {
        ...
        if ((consensus === ConsensusAlgorithm.Ethash || consensus === ConsensusAlgorithm.Clique) &&
    ...
    async handleNewBlock(block, peer) {
        ...
        try {
            ...
            this.config.logger?.debug(`Error processing new block from peer ${peer ? `id=${peer.id.slice(0, 8)}` : '(no peer)'} hash=${short(block.hash())}`);
