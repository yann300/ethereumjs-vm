...
export class SnapSynchronizer extends Synchronizer {
    constructor(options) {
        super(options);
        this.running = false;
        this.fetcherDoneFlags = getInitFetcherDoneFlags();
    ...
    async checkAndSync() {
        ...
        if (snapTargetHeight === undefined ||
            ...
            throw Error(`Invalid synced data by snapsync snapTargetHeight=${snapTargetHeight} snapTargetRoot=${short(snapTargetRoot ?? 'na')} snapTargetHash=${short(snapTargetHash ?? 'na')}`);
        }
        ...
        if (!equalsBytes(syncedRoot, snapTargetRoot)) {
            throw Error(`Invalid snap syncedRoot=${short(syncedRoot)} targetRoot=${short(snapTargetRoot)}  for target height=${snapTargetHeight} hash=${short(snapTargetHash)}`);
        ...
        const snapDoneMsg = `snapsync complete!!! height=${snapTargetHeight} root=${short(snapTargetRoot)}  hash=${short(snapTargetHash)}`;
        if (fetchingAlreadyDone) {
    ...
    async syncWithPeer(peer) {
        ...
        if ((this.fetcher === null || this.fetcher.syncErrored !== undefined) &&
            ...
            this.config.logger?.info(`syncWithPeer new AccountFetcher peer=${peer?.id} snapTargetHeight=${this.fetcherDoneFlags.snapTargetHeight} snapTargetRoot=${short(this.fetcherDoneFlags.snapTargetRoot)}  ${this.fetcher === null
                ? ''
