...
export class BeaconSynchronizer extends Synchronizer {
    ...
    async open() {
        ...
        this.config.logger?.info(`Latest local block number=${Number(number)} td=${td} hash=${bytesToHex(hash)} hardfork=${this.config.chainCommon.hardfork()}`);
    ...
    async reorged(block) {
        ...
        await this.stop();
        this.config.logger?.debug(`Beacon sync reorged, new head number=${block.header.number} hash=${short(block.header.hash())}`);
        void this.start();
    }
    /**
