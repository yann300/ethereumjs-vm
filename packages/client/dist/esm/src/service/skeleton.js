...
import { BIGINT_0, BIGINT_1, BIGINT_2EXP256, BIGINT_100, EthereumJSErrorWithoutCode, Lock, bigIntToBytes, bytesToBigInt, bytesToInt, equalsBytes, formatBigDecimal, intToBytes, utf8ToBytes, } from '@ethereumjs/util';
...
export class Skeleton extends MetaDBManager {
    constructor(opts) {
        ...
        this.syncedchain = 0;
        this.pulled = BIGINT_0; /** Number of headers downloaded in this run */
        this.filling = false; /** Whether we are actively filling the canonical chain */
        this.lastfilledAt = 0;
        this.lastfilled = BIGINT_0;
        this.lastexecutedAt = 0;
        this.lastexecuted = BIGINT_0;
        this.lastfetchedAt = 0;
        this.lastfetched = BIGINT_0;
        this.lastvalid = 0;
        this.lastFcuTime = 0;
        this.lastsyncedAt = 0;
    ...
    async processNewHead(head, force = false) {
        ...
        if (number === BIGINT_0) {
            if (!equalsBytes(this.chain.genesis.hash(), head.hash())) {
                throw Error(`Invalid genesis setHead announcement number=${number} hash=${short(head.hash())} genesisHash=${short(this.chain.genesis.hash())}`);
            }
        ...
        if (lastchain.tail > number) {
            ...
            if (mayBeDupBlock !== undefined && equalsBytes(mayBeDupBlock.header.hash(), head.hash())) {
                this.config.logger?.debug(`Skeleton duplicate ${force ? 'setHead' : 'announcement'} tail=${lastchain.tail} head=${lastchain.head} number=${number} hash=${short(head.hash())}`);
                return false;
            }
            else {
                ...
                if (force) {
                    this.config.logger?.debug(`Skeleton head reorg tail=${lastchain.tail} head=${lastchain.head} number=${number} expected=${short(mayBeDupBlock?.hash() ?? zeroBlockHash)} actual=${short(head.hash())}`);
                }
                else {
        ...
        if (parent === undefined || !equalsBytes(parent.hash(), head.header.parentHash)) {
            if (force) {
                this.config.logger?.warn(`Beacon chain forked ancestor=${parent?.header.number} hash=${short(parent?.hash() ?? 'NA')} want=${short(head.header.parentHash)}`);
            }
            return true;
        }
        if (force) {
            ...
            this.config.logger?.debug(`Beacon chain extended new head=${lastchain.head} tail=${lastchain.tail} next=${short(lastchain.next)}`);
        }
        return false;
    }
    /**
    ...
    async setHead(head, force = true, init = false, reorgThrow = false) {
        ...
        return this.runWithLock(async () => {
            ...
            this.config.logger?.debug(`New skeleton head announced number=${head.header.number} hash=${short(head.hash())} force=${force}`);
            ...
            if (lastchain === undefined) {
                ...
                this.status.linked = true;
                this.status.canonicalHeadReset = false;
                this.config.logger?.debug(`Initing empty skeleton with current chain head tail=${lastchain.tail} head=${lastchain.head} next=${short(lastchain.next)}`);
                this.status.progress.subchains.push(lastchain);
            }
            ...
            if (force && reorg) {
                ...
                if (subchain === undefined ||
                    ...
                    if (subchain !== undefined &&
                        ...
                        const truncateTailTo = truncateTailToNumber <= subchain.head
                        ...
                        if (truncateTailTo !== undefined) {
                            ...
                            this.config.logger?.info(`Truncated subchain0 with head=${subchain.head} to a new tail=${subchain.tail} next=${short(subchain.next)} before overlaying a new subchain`);
                        }
                        else {
                            // clear out this subchain
                            this.config.logger?.info(`Dropping subchain0 with head=${subchain.head} before overlaying a new subchain as truncateTailToNumber=${truncateTailToNumber} block not available `);
                    ...
                    if (this.status.linked &&
                        ...
                        if (truncateTailTo === undefined) {
                            ...
                            this.config.logger?.info(`Truncated subchain tail for chain reorg to the subchain head=${subchain.tail} next=${short(subchain.next)} linked=${this.status.linked} canonicalHeadReset=${this.status.canonicalHeadReset}`);
                        }
                        else {
                            subchain.tail = truncateTailTo.header.number;
                            subchain.next = truncateTailTo.header.parentHash;
                            // just reset tail and no need to modify linked status
                            this.config.logger?.info(`Truncated subchain with head=${subchain.head} to a new tail=${subchain.tail} next=${short(subchain.next)} linked=${this.status.linked} canonicalHeadReset=${this.status.canonicalHeadReset}`);
                        }
                    }
                }
            ...
            if (height >= (this.config.syncTargetHeight ?? BIGINT_0)) {
                ...
                if (diff < this.config.syncedStateRemovalPeriod) {
                    if (!this.synchronized) {
                        ...
                        this.config.superMsg(`Synchronized cl (skeleton) at height=${height} hash=${short(latest.hash())} 🎉`);
                    }
                }
            }
        }
    ...
    async forkchoiceUpdate(headBlock, { safeBlockHash, finalizedBlockHash, } = {}) {
        ...
        await this.runWithLock(async () => {
            ...
            if ((finalizedBlock?.header.number ?? BIGINT_0) >
    ...
    async trySubChainsMerge() {
        ...
        while (this.status.progress.subchains.length > 1 &&
            ...
            if (tail >= this.status.progress.subchains[0].tail) {
                ...
                this.config.logger?.debug(`Previous subchain fully overwritten tail=${tail} head=${head} next=${short(next)}`);
                ...
                this.config.logger?.debug(`Previous subchain partially overwritten tail=${tail} head=${head} next=${short(next)} with newHead=${this.status.progress.subchains[1].head}`);
                edited = true;
            }
            ...
            if (subChain1Head === undefined ||
            ...
            else if (equalsBytes(subChain1Head.hash(), this.status.progress.subchains[0].next) === true) {
                ...
                if (head - tail > this.config.skeletonSubchainMergeMinimum) {
                    this.config.logger?.debug(`Previous subchain merged tail=${tail} head=${head} next=${short(next)}`);
    ...
    async putBlocks(blocks, skipForwardFill = false) {
        return this.runWithLock(async () => {
            ...
            this.config.logger?.debug(`Skeleton putBlocks start=${blocks[0]?.header.number} hash=${short(blocks[0]?.hash())} fork=${blocks[0].common.hardfork()} end=${blocks[blocks.length - 1]?.header.number} count=${blocks.length}, subchain head=${this.status.progress.subchains[0]?.head} tail = ${this.status.progress.subchains[0].tail} next=${short(this.status.progress.subchains[0]?.next)}`);
            for (const block of blocks) {
                ...
                if (number >= this.status.progress.subchains[0].tail) {
                    ...
                    if (!equalsBytes(this.chain.genesis.hash(), block.hash())) {
                        throw Error(`Skeleton pubBlocks with invalid genesis block number=${number} hash=${short(block.hash())} genesisHash=${short(this.chain.genesis.hash())}`);
                    }
                    continue;
                }
                else if (number < BIGINT_0) {
    ...
    async blockingFillWithCutoff(cutoffLen) {
        ...
        if (this.status.linked && subchain0 !== undefined) {
            const isChainHeadNearEnough = subchain0.head - BigInt(cutoffLen) <
    ...
    async fillCanonicalChain(skipUpdateEmit = false) {
        ...
        this.config.logger?.debug(`Starting canonical chain fill canonicalHead=${canonicalHead} subchainHead=${subchain.head} skipUpdateEmit=${skipUpdateEmit}`);
        ...
        while (!this.status.canonicalHeadReset && canonicalHead < subchain.head) {
            ...
            if (this.chain.blocks.height <= block.header.number) {
                ...
                if (numBlocksInserted !== 1) {
                    this.config.logger?.error(`Failed to put block number=${number} fork=${block.common.hardfork()} hash=${short(block.hash())} parentHash=${short(block.header.parentHash)}from skeleton chain to canonical`);
                    ...
                    try {
                        ...
                        this.config.logger?.info(`ParentByNumber number=${parent?.header.number}, hash=${short(parent?.hash() ?? 'undefined')} hf=${parent?.common.hardfork()}`);
                    }
                    catch {
                    ...
                    try {
                        ...
                        this.config.logger?.info(`parentByHash number=${parentWithHash?.header.number}, hash=${short(parentWithHash?.hash() ?? 'undefined')} hf=${parentWithHash?.common.hardfork()}  `);
                    }
                    catch {
                        this.config.logger?.error(`Failed to fetch parent with parentWithHash=${short(block.header.parentHash)}`);
                    }
                    break;
                }
            }
    ...
    deserialize(rlp) {
        const [hardfork, blockRLP] = RLP.decode(rlp);
    ...
    async putBlock(block, onlyUnfinalized = false) {
        ...
        if (!onlyUnfinalized) {
            ...
            await this.put(DBKey.SkeletonBlockHashToNumber, block.hash(), bigIntToBytes(block.header.number));
        }
        return true;
    }
    ...
    logSyncStatus(logPrefix, { forceShowInfo, lastStatus, vmexecution, fetching, snapsync, peers, } = {}) {
        ...
        switch (status) {
            ...
            case 'SYNCED':
                if (vmexecution?.started === true) {
                    ...
                    if (snapsync.done) {
                        scenario = `snapsync-to-vm-transition=${(snapsync.snapTargetHeight ?? BIGINT_0) + this.config.snapTransitionSafeDepth}`;
                    }
                    else {
        ...
        const chainHead = `el=${this.chain.blocks.latest?.header.number ?? 'na'} hash=${short(this.chain.blocks.latest?.hash() ?? 'na')}`;
        ...
        if (forceShowInfo || status !== lastStatus) {
            ...
            if (isValid) {
                ...
                vmlogInfo = `vm=${vmHead?.header.number} hash=${short(vmHead?.hash() ?? 'na')} started=${vmexecution?.started}`;
                if (vmexecution?.started === true) {
                    ...
                    if (snapsync === undefined) {
                        ...
                        if (snapsync.done === true) {
                            snapLogInfo = `snapsync=synced height=${snapTargetHeight} hash=${short(snapTargetHash ?? 'na')} root=${short(snapTargetRoot ?? 'na')}`;
                        }
                        else if (snapsync.syncing) {
                            const accountsDone = formatBigDecimal(snapsync.accountFetcher.first * BIGINT_100, BIGINT_2EXP256, BIGINT_100);
                            const storageReqsDone = formatBigDecimal(snapsync.storageFetcher.first * BIGINT_100, snapsync.storageFetcher.count, BIGINT_100);
                            const codeReqsDone = formatBigDecimal(snapsync.byteCodeFetcher.first * BIGINT_100, snapsync.byteCodeFetcher.count, BIGINT_100);
                            ...
                            snapLogInfo = `${stage} ${snapprogress} (hash=${short(snapTargetHash ?? 'na')} root=${short(snapTargetRoot ?? 'na')})`;
                        }
                        else {
                            if (this.synchronized) {
                ...
                if (!isSynced) {
                    ...
                    subchainLog = `subchains(${subchainLen}) linked=${this.status.linked} ${this.status.progress.subchains
                        ...
                        (subchain0?.tail ?? BIGINT_0) <= this.chain.blocks.height} synchronized=${this.synchronized}`;
                }
            }
    ...
    statusRLPtoObject(serializedStatus) {
        ...
        const rawStatus = RLP.decode(serializedStatus);
