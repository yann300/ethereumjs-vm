...
import { BIGINT_0, BIGINT_1, BIGINT_2, BIGINT_256 } from '@ethereumjs/util';
...
export class Config {
    constructor(options = {}) {
        ...
        this.lastSyncDate = 0;
        const common = options.common ?? new Common({ chain: Config.CHAIN_DEFAULT, hardfork: Hardfork.Chainstart });
            ...
            if (height >= (this.syncTargetHeight ?? BIGINT_0)) {
                ...
                if (diff < this.syncedStateRemovalPeriod) {
                    if (!this.synchronized) {
                        ...
                        this.superMsg(`Synchronized blockchain at height=${height} hash=${short(latest.hash())} 🎉`);
                    }
    ...
    static getConfigDB(networkDir) {
        return new Level(`${networkDir}/config`);
    }
    /**
...
Config.MAX_ACCOUNT_RANGE = (BIGINT_2 ** BIGINT_256 - BIGINT_1) / BigInt(1000);
// Larger ranges used for storage slots since assumption is slots should be much sparser than accounts
Config.MAX_STORAGE_RANGE = (BIGINT_2 ** BIGINT_256 - BIGINT_1) / BigInt(10);
...
Config.ENGINE_PARENT_LOOKUP_MAX_DEPTH = 128;
Config.ENGINE_NEWPAYLOAD_MAX_EXECUTE = 2;
Config.ENGINE_NEWPAYLOAD_MAX_TXS_EXECUTE = 200;
...
Config.BLOBS_AND_PROOFS_CACHE_BLOCKS = 32;
