import { createSealedCliqueBlockHeader } from '@ethereumjs/block';
...
import { IndexOperation, IndexType } from "../execution/txIndex.js";
/**
 * @module miner
 */
/**
...
export class Miner {
    ...
    constructor(options) {
        ...
        if (this.config.chainCommon.consensusType() === ConsensusType.ProofOfWork) {
            this.ethash = new Ethash(new LevelDB(new MemoryLevel()));
        }
    }
    /**
    ...
    async assembleBlock() {
        ...
        this.config.logger?.info(`Miner: Assembling block from ${txs.length} eligible txs ${typeof baseFeePerGas === 'bigint' && baseFeePerGas !== BIGINT_0
