...
import { BIGINT_1, BIGINT_2, TypeOutput, bigIntToUnpaddedBytes, bytesToHex, concatBytes, createZeroAddress, equalsBytes, toType, } from '@ethereumjs/util';
...
export class PendingBlock {
    constructor(opts) {
        ...
        this.executionRequests = new Map();
        /**
    ...
    async start(vm, parentBlock, headerData = {}, withdrawals) {
        ...
        if (withdrawals !== undefined && withdrawals !== null) {
            ...
            for (const withdrawal of withdrawals) {
                ...
                const validatorIndex = bigIntToUnpaddedBytes(toType(withdrawal.validatorIndex ?? 0, TypeOutput.BigInt));
        ...
        const payloadIdBytes = keccakFunction(concatBytes(parentBlock.hash(), mixHashBuf, timestampBuf, gasLimitBuf, parentBeaconBlockRootBuf, coinbaseBuf, withdrawalsBuf)).subarray(0, 8);
     ...
     */
    stop(payloadIdBytes) {
        const payloadId = typeof payloadIdBytes !== 'string' ? bytesToHex(payloadIdBytes) : payloadIdBytes;
    ...
    async build(payloadIdBytes) {
        ...
        const txs = (await this.txPool.txsByPriceAndNonce(vm, {
            ...
            allowedBlobs,
        })).filter((tx) => builder.transactions.some((t) => equalsBytes(t.hash(), tx.hash())) === false);
        ...
        this.config.logger?.info(`Pending: Built block number=${block.header.number} txs=${block.transactions.length}${withdrawalsStr}${blobsStr} skippedByAddErrors=${skippedByAddErrors}  hash=${bytesToHex(block.hash())}`);
    ...
    async addTransaction(builder, tx) {
        ...
        try {
            ...
            if (error.message === 'tx has a higher gas limit than the remaining gas in the block') {
                ...
                this.config.logger?.error(`Pending: Removed from txPool a blob tx ${bytesToHex(tx.hash())} with missing blobs`);
                addTxResult = AddTxResult.RemovedByErrors;
            }
            else {
                // If there is an error adding a tx, it will be skipped
                this.config.logger?.debug(`Pending: Skipping tx ${bytesToHex(tx.hash())}, error encountered when trying to add tx:\n${error}`);
