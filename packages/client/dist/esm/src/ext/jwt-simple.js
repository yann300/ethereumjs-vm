 ...
 * @api public
 */
const decode = function jwt_decode(token, key, noVerify = false, algorithm = '') {
    // check token
    if (!token) {
 ...
 * @api public
 */
const encode = function jwt_encode(payload, key, algorithm = '', options = undefined) {
    // Check key
    if (!key) {
