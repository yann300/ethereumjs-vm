...
export class BoundProtocol {
    ...
    constructor(options) {
        ...
        this.sender.on('error', (error) => this.config.events.emit(Event.PROTOCOL_ERROR, error, this.peer));
    }
    get status() {
        return this._status;
    }
    ...
    handle(incoming) {
        ...
        if (resolver !== undefined) {
            ...
            if (error) {
                ...
                this.config.events.emit(Event.PROTOCOL_MESSAGE, { name: message.name, data }, this.protocol.name, this.peer);
            }
        }
    }
    /**
...
export class BoundEthProtocol extends BoundProtocol {
    constructor(options) {
        super(options);
...
export class BoundSnapProtocol extends BoundProtocol {
    constructor(options) {
        super(options);
