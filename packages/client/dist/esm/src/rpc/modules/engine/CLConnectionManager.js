...
export class CLConnectionManager {
    ...
    _getForkchoiceUpdateLogMsg(update) {
        ...
        msg += `head=${short(update.state.headBlockHash)} finalized=${short(update.state.finalizedBlockHash)} response=${update.response ? update.response.payloadStatus.status : '-'}`;
        if (update.headBlock) {
 ...
 */
export function middleware(methodFn, handler) {
