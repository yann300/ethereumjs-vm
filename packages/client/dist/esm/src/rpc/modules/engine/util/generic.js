 ...
 */
export const recursivelyFindParents = async (vmHeadHash, parentHash, chain) => {
 ...
 */
export const validExecutedChainBlock = async (blockOrHash, chain) => {
    try {
 ...
 * Returns the block hash as a 0x-prefixed hex string if found valid in the blockchain, otherwise returns null.
 */
export const validHash = async (hash, chain, chainCache) => {
    ...
    try {
        ...
        for (let inspectedParents = 0; inspectedParents < maxDepth; inspectedParents++) {
            ...
            const isBlockExecuted = (executedBlocks.get(unPrefixedHashStr) ?? (await validExecutedChainBlock(hash, chain))) !==
                null;
            if (isBlockExecuted) {
...
export function validateHardforkRange(chainCommon, methodVersion, checkNotBeforeHf, checkNotAfterHf, timestamp) {
