...
import { BIGINT_1, EthereumJSErrorWithoutCode, bytesToHex, bytesToUnprefixedHex, equalsBytes, hexToBytes, } from '@ethereumjs/util';
...
import { INTERNAL_ERROR, INVALID_PARAMS, TOO_LARGE_REQUEST, UNSUPPORTED_FORK, validEngineCodes, } from "../../error-code.js";
...
import { assembleBlock, blockToExecutionPayload, getPayloadBody, pruneCachedBlocks, recursivelyFindParents, validExecutedChainBlock, validHash, validateHardforkRange, } from "./util/index.js";
import { executionPayloadV1FieldValidators, executionPayloadV2FieldValidators, executionPayloadV3FieldValidators, forkchoiceFieldValidators, payloadAttributesFieldValidatorsV1, payloadAttributesFieldValidatorsV2, payloadAttributesFieldValidatorsV3, } from "./validators.js";
...
export class Engine {
    ...
    initValidators() {
         ...
         * newPayload
         */
        this.newPayloadV1 = cmMiddleware(middleware(callWithStackTrace(this.newPayloadV1.bind(this), this._rpcDebug), 1, [
        ...
        this.newPayloadV2 = cmMiddleware(middleware(callWithStackTrace(this.newPayloadV2.bind(this), this._rpcDebug), 1, [
            [
                validators.either(validators.object(executionPayloadV1FieldValidators), validators.object(executionPayloadV2FieldValidators)),
            ],
        ]), ([payload], response) => this.connectionManager.lastNewPayload({ payload, response }));
        this.newPayloadV3 = cmMiddleware(middleware(callWithStackTrace(this.newPayloadV3.bind(this), this._rpcDebug), 3, [
        ...
        this.newPayloadV4 = cmMiddleware(middleware(callWithStackTrace(this.newPayloadV4.bind(this), this._rpcDebug), 4, [
            [validators.object(executionPayloadV3FieldValidators)],
            [validators.array(validators.bytes32)],
            [validators.bytes32],
            [validators.array(validators.hex)],
        ], ['executionPayload', 'blobVersionedHashes', 'parentBeaconBlockRoot', 'executionRequests']), ([payload], response) => this.connectionManager.lastNewPayload({ payload, response }));
        /**
         * forkchoiceUpdated
         */
        ...
        this.forkchoiceUpdatedV1 = cmMiddleware(middleware(callWithStackTrace(this.forkchoiceUpdatedV1.bind(this), this._rpcDebug), 1, [
        ...
        this.forkchoiceUpdatedV2 = cmMiddleware(middleware(callWithStackTrace(this.forkchoiceUpdatedV2.bind(this), this._rpcDebug), 1, [
            [validators.object(forkchoiceFieldValidators)],
            [validators.optional(validators.object(payloadAttributesFieldValidatorsV2))],
        ]), forkchoiceUpdatedResponseCMHandler);
        this.forkchoiceUpdatedV3 = cmMiddleware(middleware(callWithStackTrace(this.forkchoiceUpdatedV3.bind(this), this._rpcDebug), 1, [
            [validators.object(forkchoiceFieldValidators)],
         ...
         * getPayload
         */
        this.getPayloadV1 = cmMiddleware(middleware(callWithStackTrace(this.getPayloadV1.bind(this), this._rpcDebug), 1, [
            [validators.bytes8],
        ]), () => this.connectionManager.updateStatus());
        this.getPayloadV2 = cmMiddleware(middleware(callWithStackTrace(this.getPayloadV2.bind(this), this._rpcDebug), 1, [
            [validators.bytes8],
        ]), () => this.connectionManager.updateStatus());
        this.getPayloadV3 = cmMiddleware(middleware(callWithStackTrace(this.getPayloadV3.bind(this), this._rpcDebug), 1, [
            [validators.bytes8],
        ]), () => this.connectionManager.updateStatus());
        this.getPayloadV4 = cmMiddleware(middleware(callWithStackTrace(this.getPayloadV4.bind(this), this._rpcDebug), 1, [
            [validators.bytes8],
        ]), () => this.connectionManager.updateStatus());
        /**
         * exchangeCapabilities
         */
        this.exchangeCapabilities = cmMiddleware(middleware(callWithStackTrace(this.exchangeCapabilities.bind(this), this._rpcDebug), 0, []), () => this.connectionManager.updateStatus());
        /**
         * getPayloadBodiesByHash
         */
        this.getPayloadBodiesByHashV1 = cmMiddleware(middleware(callWithStackTrace(this.getPayloadBodiesByHashV1.bind(this), this._rpcDebug), 1, [
            [validators.array(validators.bytes32)],
        ]), () => this.connectionManager.updateStatus());
        /**
         * getPayloadBodiesByRange
         */
        this.getPayloadBodiesByRangeV1 = cmMiddleware(middleware(callWithStackTrace(this.getPayloadBodiesByRangeV1.bind(this), this._rpcDebug), 2, [
            [validators.bytes8],
            [validators.bytes8],
        ]), () => this.connectionManager.updateStatus());
        this.getBlobsV1 = cmMiddleware(middleware(callWithStackTrace(this.getBlobsV1.bind(this), this._rpcDebug), 1, [
            [validators.array(validators.bytes32)],
        ]), () => this.connectionManager.updateStatus());
    }
    /**
    ...
    async newPayload(params) {
        ...
        const { block: headBlock, error } = await assembleBlock(payload, {
        ...
        if (hardfork !== this.lastNewPayloadHF && this.lastNewPayloadHF !== '') {
            this.config.logger?.info(`Hardfork change along new payload block number=${headBlock.header.number} hash=${short(headBlock.hash())} old=${this.lastNewPayloadHF} new=${hardfork}`);
        }
        ...
        try {
             ...
             */
            const parent = (await this.skeleton.getBlockByHash(hexToBytes(parentHash), true)) ??
            ...
            if (!executedParentExists) {
                throw EthereumJSErrorWithoutCode(`Parent block not yet executed number=${parent.header.number}`);
            }
        }
        catch {
            ...
            if (this.execution.chainStatus?.status === ExecStatus.INVALID &&
                ...
                if (invalidBlock !== undefined) {
                    ...
                    const latestValidHash = await validHash(invalidBlock.header.parentHash, this.chain, this.chainCache);
                    const validationError = `Block number=${invalidBlock.header.number} hash=${short(invalidBlock.hash())} root=${short(invalidBlock.header.stateRoot)} along the canonical chain is invalid`;
                    const response = {
                        status: Status.INVALID,
                        latestValidHash,
                        validationError,
        ...
        try {
            for (const [i, block] of blocks.entries()) {
                ...
                const isBlockExecuted = (this.executedBlocks.get(bytesToUnprefixedHex(bHash)) ??
                ...
                if (!isBlockExecuted) {
                    ...
                    const executed = shouldExecuteBlock &&
                        (await (async () => {
                            ...
                            const blockParent = i > 0
                                ? blocks[i - 1]
                                : (this.chainCache.remoteBlocks.get(bytesToHex(block.header.parentHash).slice(2)) ?? (await this.chain.getBlock(block.header.parentHash)));
                    ...
                    if (!executed) {
                        this.config.logger?.debug(`Skipping block(s) execution for headBlock=${headBlock.header.number} hash=${short(headBlock.hash())} : pendingBlocks=${blocks.length - i}(limit=${this.chain.config.engineNewpayloadMaxExecute}) transactions=${block.transactions.length}(limit=${this.chain.config.engineNewpayloadMaxTxsExecute}) executionBusy=${this.execution.running}`);
            ...
            const latestValidHash = await validHash(headBlock.header.parentHash, this.chain, this.chainCache);
    ...
    async forkchoiceUpdated(params) {
        ...
        try {
            ...
            this.config.logger?.debug(`Forkchoice announced head block unknown to EL hash=${short(headBlockHash)}`);
            const payloadStatus = {
        ...
        if (hardfork !== this.lastForkchoiceUpdatedHF && this.lastForkchoiceUpdatedHF !== '') {
            this.config.logger?.info(`Hardfork change along forkchoice head block update number=${headBlock.header.number} hash=${short(headBlock.hash())} old=${this.lastForkchoiceUpdatedHF} new=${hardfork}`);
        }
        ...
        this.config.logger?.debug(`Forkchoice requested update to new head number=${headBlock.header.number} hash=${short(headBlock.hash())}`);
        /**
        ...
        if (!isHeadExecuted) {
            ...
            if (this.execution.chainStatus?.status === ExecStatus.IGNORE_INVALID &&
                this.config.ignoreStatelessInvalidExecs === true) {
                // jump the vm head to failing block so that next block can be executed
                this.config.logger?.debug(`Jumping the stalled vmHead forward to hash=${this.execution.chainStatus.hash} height=${this.execution.chainStatus.height} to continue the execution`);
                await this.execution.jumpVmHead(this.execution.chainStatus.hash, this.execution.chainStatus.height);
            }
        ...
        if (!equalsBytes(vmHeadHash, headBlock.hash())) {
            ...
            if (this.chain.headers.latest && this.chain.headers.latest.number < headBlock.header.number) {
                try {
                    parentBlocks = await recursivelyFindParents(vmHeadHash, headBlock.header.parentHash, this.chain);
                }
                catch {
                    const payloadStatus = {
                        status: Status.SYNCING,
        ...
        if (payloadAttributes) {
            ...
            if (timestampBigInt <= headBlock.header.timestamp) {
                throw {
                    message: `invalid timestamp in payloadAttributes, got ${timestampBigInt}, need at least ${headBlock.header.timestamp + BIGINT_1}`,
                    code: INVALID_PARAMS,
                };
            }
    ...
    async forkchoiceUpdatedV1(params) {
        ...
        if (payloadAttributes !== undefined && payloadAttributes !== null) {
            ...
            validateHardforkRange(this.chain.config.chainCommon, 1, null, Hardfork.Paris, BigInt(payloadAttributes.timestamp));
        }
    ...
    async forkchoiceUpdatedV2(params) {
        ...
        if (payloadAttributes !== undefined && payloadAttributes !== null) {
            ...
            validateHardforkRange(this.chain.config.chainCommon, 2, null, Hardfork.Shanghai, BigInt(payloadAttributes.timestamp));
            const shanghaiTimestamp = this.chain.config.chainCommon.hardforkTimestamp(Hardfork.Shanghai);
    ...
    async forkchoiceUpdatedV3(params) {
        ...
        if (payloadAttributes !== undefined && payloadAttributes !== null) {
            ...
            validateHardforkRange(this.chain.config.chainCommon, 3, Hardfork.Cancun, 
    ...
    async getPayload(params, payloadVersion) {
        ...
        try {
            ...
            validateHardforkRange(this.chain.config.chainCommon, payloadVersion, checkNotBeforeHf, checkNotAfterHf, BigInt(executionPayload.executionPayload.timestamp));
            return executionPayload;
        }
        catch (error) {
            if (validEngineCodes.includes(error.code))
                throw error;
            throw {
                code: INTERNAL_ERROR,
