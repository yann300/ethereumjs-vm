...
import { CLRequest, CLRequestType, EthereumJSErrorWithoutCode, bytesToHex, hexToBytes, } from '@ethereumjs/util';
import { sha256 } from 'ethereum-cryptography/sha256.js';
import { short } from "../../../../util/index.js";
import { Status } from "../types.js";
import { validHash } from "./generic.js";
export const validate4844BlobVersionedHashes = (headBlock, blobVersionedHashes) => {
    ...
    if (blobVersionedHashes.length !== txVersionedHashes.length) {
        ...
        for (let vIndex = 0; vIndex < blobVersionedHashes.length; vIndex++) {
            ...
            if (blobVersionedHashes[vIndex] !== txVersionedHashes[vIndex]) {
                validationError = `Error verifying blobVersionedHashes: mismatch at index=${vIndex} expected=${short(txVersionedHashes[vIndex])} received=${short(blobVersionedHashes[vIndex])}`;
                break;
            }
        }
    }
...
export const validateAndGen7685RequestsHash = (common, executionRequests) => {
    const requests = [];
 ...
 */
export const assembleBlock = async (payload, clValidationData, chain, chainCache) => {
    ...
    try {
        ...
        const block = await createBlockFromExecutionPayload({ ...payload, parentBeaconBlockRoot, requestsHash }, { common });
