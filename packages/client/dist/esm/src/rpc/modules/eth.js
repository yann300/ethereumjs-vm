...
import { Capability, NetworkWrapperType, createBlob4844TxFromSerializedNetworkWrapper, createTx, createTxFromRLP, } from '@ethereumjs/tx';
import { BIGINT_0, BIGINT_1, BIGINT_100, BIGINT_NEG1, EthereumJSErrorWithoutCode, TypeOutput, bigIntMax, bigIntToHex, bytesToHex, createAddressFromString, createZeroAddress, equalsBytes, hexToBytes, intToHex, isHexString, setLengthLeft, toType, } from '@ethereumjs/util';
 ...
 */
const toJSONRPCBlock = async (block, chain, includeTransactions) => {
    const json = block.toJSON();
    const header = json.header;
    const transactions = block.transactions.map((tx, txIndex) => includeTransactions ? toJSONRPCTx(tx, block, txIndex) : bytesToHex(tx.hash()));
 ...
 */
const toJSONRPCLog = async (log, block, tx, txIndex, logIndex) => ({
 ...
 */
const toJSONRPCReceipt = async (receipt, gasUsed, effectiveGasPrice, block, tx, txIndex, logIndex, contractAddress, blobGasUsed, blobGasPrice) => ({
    ...
    logs: await Promise.all(receipt.logs.map((l, i) => toJSONRPCLog(l, block, tx, txIndex, logIndex + i))),
    logsBloom: bytesToHex(receipt.bitvector),
    root: receipt.stateRoot instanceof Uint8Array
        ? bytesToHex(receipt.stateRoot)
        : undefined,
    status: receipt.status !== undefined
        ? intToHex(receipt.status)
        : undefined,
...
const calculateRewards = async (block, receiptsManager, priorityFeePercentiles) => {
    ...
    // Loop over all txs ...
    let targetCumulativeGasUsed = (block.header.gasUsed * BigInt(priorityFeePercentiles[0])) / BIGINT_100;
...
export class Eth {
    ...
    constructor(client, rpcDebug) {
        ...
        this.txIndex = this.service.execution?.txIndex;
        ...
        this.maxPriorityFeePerGas = callWithStackTrace(this.maxPriorityFeePerGas.bind(this), this._rpcDebug);
        this.estimateGas = middleware(callWithStackTrace(this.estimateGas.bind(this), this._rpcDebug), 1, [[validators.transaction()], [validators.blockOption]]);
        this.getBalance = middleware(callWithStackTrace(this.getBalance.bind(this), this._rpcDebug), 2, [[validators.address], [validators.blockOption]]);
        this.coinbase = callWithStackTrace(this.coinbase.bind(this), this._rpcDebug);
        this.getBlockByNumber = middleware(callWithStackTrace(this.getBlockByNumber.bind(this), this._rpcDebug), 2, [[validators.blockOption], [validators.bool]]);
        this.getBlockByHash = middleware(callWithStackTrace(this.getBlockByHash.bind(this), this._rpcDebug), 2, [[validators.hex, validators.blockHash], [validators.bool]]);
        this.getBlockTransactionCountByHash = middleware(callWithStackTrace(this.getBlockTransactionCountByHash.bind(this), this._rpcDebug), 1, [[validators.hex, validators.blockHash]]);
        this.getCode = middleware(callWithStackTrace(this.getCode.bind(this), this._rpcDebug), 2, [
            [validators.address],
            [validators.blockOption],
        ]);
        this.getUncleCountByBlockNumber = middleware(callWithStackTrace(this.getUncleCountByBlockNumber.bind(this), this._rpcDebug), 1, [[validators.hex]]);
        this.getStorageAt = middleware(callWithStackTrace(this.getStorageAt.bind(this), this._rpcDebug), 3, [[validators.address], [validators.hex], [validators.blockOption]]);
        this.getTransactionByBlockHashAndIndex = middleware(callWithStackTrace(this.getTransactionByBlockHashAndIndex.bind(this), this._rpcDebug), 2, [[validators.hex, validators.blockHash], [validators.hex]]);
        this.getTransactionByBlockNumberAndIndex = middleware(callWithStackTrace(this.getTransactionByBlockNumberAndIndex.bind(this), this._rpcDebug), 2, [[validators.hex, validators.blockOption], [validators.hex]]);
        this.getTransactionByHash = middleware(callWithStackTrace(this.getTransactionByHash.bind(this), this._rpcDebug), 1, [[validators.hex]]);
        this.getTransactionCount = middleware(callWithStackTrace(this.getTransactionCount.bind(this), this._rpcDebug), 2, [[validators.address], [validators.blockOption]]);
        this.getBlockReceipts = middleware(callWithStackTrace(this.getBlockReceipts.bind(this), this._rpcDebug), 1, [[validators.blockOption]]);
        this.getTransactionReceipt = middleware(callWithStackTrace(this.getTransactionReceipt.bind(this), this._rpcDebug), 1, [[validators.hex]]);
        this.getUncleCountByBlockNumber = middleware(callWithStackTrace(this.getUncleCountByBlockNumber.bind(this), this._rpcDebug), 1, [[validators.hex]]);
        this.getLogs = middleware(callWithStackTrace(this.getLogs.bind(this), this._rpcDebug), 1, [
            [
                validators.object({
                    ...
                    address: validators.optional(validators.either(validators.array(validators.address), validators.address)),
                    topics: validators.optional(validators.array(validators.optional(validators.either(validators.hex, validators.array(validators.hex))))),
                    blockHash: validators.optional(validators.blockHash),
                }),
            ],
        ]);
        this.sendRawTransaction = middleware(callWithStackTrace(this.sendRawTransaction.bind(this), this._rpcDebug), 1, [[validators.hex]]);
        ...
        this.getBlockTransactionCountByNumber = middleware(callWithStackTrace(this.getBlockTransactionCountByNumber.bind(this), this._rpcDebug), 1, [[validators.blockOption]]);
        this.gasPrice = callWithStackTrace(this.gasPrice.bind(this), this._rpcDebug);
        this.feeHistory = middleware(callWithStackTrace(this.feeHistory.bind(this), this._rpcDebug), 2, [
    ...
    async getStorageAt(params) {
        ...
        return storage !== null && storage !== undefined
            ? bytesToHex(setLengthLeft(Uint8Array.from(storage), 32))
            : EMPTY_SLOT;
    }
    /**
    ...
    async getTransactionByHash(params) {
        ...
        if (!this.txIndex)
            throw EthereumJSErrorWithoutCode('missing txIndex');
    ...
    async getTransactionCount(params) {
        ...
        if (blockOpt !== 'pending')
            ...
            block = await getBlockByOption('latest', this._chain);
        if (this._vm === undefined) {
            throw EthereumJSErrorWithoutCode('missing vm');
        }
        const vm = await this._vm.shallowCopy();
        ...
        if (blockOpt === 'pending') {
            pendingTxsCount = BigInt(this.service.txPool.pool.get(addressHex.slice(2))?.length ?? 0);
        }
    ...
    async getBlockReceipts(params) {
        ...
        const receipts = await Promise.all(result.map(async (r, i) => {
            const tx = block.transactions[i];
            const { totalGasSpent, createdAddress } = runBlockResult.results[i];
            const { blobGasPrice, blobGasUsed } = runBlockResult.receipts[i];
            const effectiveGasPrice = tx.supports(Capability.EIP1559FeeMarket) === true
                ? tx.maxPriorityFeePerGas <
                    ...
                    : tx.maxFeePerGas -
                        ...
                        block.header.baseFeePerGas
                : tx.gasPrice;
            return toJSONRPCReceipt(r, totalGasSpent, effectiveGasPrice, block, tx, i, i, createdAddress, blobGasUsed, blobGasPrice);
        }));
        return receipts;
    }
    /**
    ...
    async getTransactionReceipt(params) {
        ...
        return toJSONRPCReceipt(receipt, totalGasSpent, effectiveGasPrice, block, tx, txIndex, logIndex, createdAddress, blobGasUsed, blobGasPrice);
    }
    /**
    ...
    async getLogs(params) {
        ...
        return Promise.all(logs.map(({ log, block, tx, txIndex, logIndex }) => toJSONRPCLog(log, block, tx, txIndex, logIndex)));
    }
    /**
        ...
        try {
            ...
            if (txBuf[0] === 0x03) {
                ...
                if (common.isActivatedEIP(7594) &&
                    ...
                    throw Error(`tx with networkWrapperVersion=${tx.networkWrapperVersion} sent for EIP-7594 activated hardfork=${common.hardfork()}`);
                }
    ...
    async getProof(params) {
        ...
        if (vm.stateManager instanceof MerkleStateManager) {
            ...
            throw EthereumJSErrorWithoutCode('getProof RPC method not supported with the StateManager provided');
        }
    ...
    async feeHistory(params) {
        ...
        const { number: lastRequestedBlockNumber } = (await getBlockByOption(lastBlockRequested, this._chain)).header;
        ...
        const requestedBlocks = await Promise.all(requestedBlockNumbers.map((n) => getBlockByOption(n.toString(), this._chain)));
        ...
        const nextBaseFee = lastRequestedBlockNumber - londonHardforkBlockNumber >= BIGINT_NEG1
