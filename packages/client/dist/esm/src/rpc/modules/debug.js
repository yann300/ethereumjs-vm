import { EthereumJSErrorWithoutCode, TypeOutput, bigIntToHex, bytesToHex, createAddressFromString, hexToBytes, toType, } from '@ethereumjs/util';
import { encodeReceipt, runTx } from '@ethereumjs/vm';
...
export class Debug {
    ...
    constructor(client, rpcDebug) {
        ...
        this.traceTransaction = middleware(callWithStackTrace(this.traceTransaction.bind(this), this._rpcDebug), 1, [[validators.hex]]);
        this.traceCall = middleware(callWithStackTrace(this.traceCall.bind(this), this._rpcDebug), 2, [
            [validators.transaction()],
            [validators.blockOption],
        ]);
        this.storageRangeAt = middleware(callWithStackTrace(this.storageRangeAt.bind(this), this._rpcDebug), 5, [
        ...
        this.getRawBlock = middleware(callWithStackTrace(this.getRawBlock.bind(this), this._rpcDebug), 1, [[validators.blockOption]]);
        this.getRawHeader = middleware(callWithStackTrace(this.getRawHeader.bind(this), this._rpcDebug), 1, [[validators.blockOption]]);
        this.getRawReceipts = middleware(callWithStackTrace(this.getRawReceipts.bind(this), this._rpcDebug), 1, [[validators.blockOption]]);
        this.getRawTransaction = middleware(callWithStackTrace(this.getRawTransaction.bind(this), this._rpcDebug), 1, [[validators.hex]]);
    ...
    async traceTransaction(params) {
        ...
        const result = await this.service.execution.receiptsManager.getReceiptByTxHash(hexToBytes(txHash));
        if (!result)
            return null;
    ...
    async storageRangeAt(params) {
        ...
        createAddressFromString(account), BigInt(startKey), limit);
    }
    /**
     * Returns an RLP-encoded block
     * @param blockOpt Block number or tag
     */
    async getRawBlock(params) {
     ...
     * @param blockOpt Block number or tag
     */
    async getRawReceipts(params) {
        const [blockOpt] = params;
        if (!this.service.execution.receiptsManager)
            throw EthereumJSErrorWithoutCode('missing receiptsManager');
        const block = await getBlockByOption(blockOpt, this.chain);
        const receipts = await this.service.execution.receiptsManager.getReceipts(block.hash(), true, true);
