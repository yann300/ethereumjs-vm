...
import { Chain, Common, Hardfork, Mainnet, createCommonFromGethGenesis, createCustomCommon, getPresetChainConfig, parseGethGenesisState, } from '@ethereumjs/common';
import { EthereumJSErrorWithoutCode, bytesToBigInt, bytesToHex, calculateSigRecovery, concatBytes, createAddressFromPrivateKey, createAddressFromString, ecrecover, hexToBytes, randomBytes, setLengthLeft, } from '@ethereumjs/util';
import { trustedSetup } from '@paulmillr/trusted-setups/fast.js';
import { keccak256 as keccak256WASM, secp256k1Expand, secp256k1Recover, secp256k1Sign, waitReady as waitReadyPolkadotSha256, sha256 as wasmSha256, } from '@polkadot/wasm-crypto';
...
export function getArgs() {
    return (yargs
        ...
        string: true,
        coerce: (customGenesisStateRoot) => hexToBytes(customGenesisStateRoot),
    })
        .option('gethGenesis', {
      ...
      FOR YOUR SAFETY PLEASE DO NOT USE ANY ACCOUNTS HOLDING SUBSTANTIAL AMOUNTS OF ETH`,
        ...
        describe: 'Block number to start/restart execution from. For merkle based state, parent state should be present in the the db while in verkle stateless mode the chain should be synced till the block and witnesses available this block onwards',
        number: true,
    })
        .option('startExecution', {
        describe: 'Start execution of unexecuted blocks without waiting for the CL fcU, set to `true` if `startExecutionFrom` provided',
        boolean: true,
    })
        .option('isSingleNode', {
        describe: 'To run client in single node configuration without need to discover the sync height from peer. Particularly useful in test configurations. This flag is automatically activated in the "dev" mode',
        boolean: true,
    })
        ...
        describe: 'Number of unexecuted blocks (including ancestors) that can be executed per-block in engine`s new payload (if required and possible) to determine the validity of the block',
        number: true,
    })
        ...
        if (argv.rpcEngine === true && usedPorts.has(argv.rpcEnginePort))
            collision = true;
        if (collision)
            throw EthereumJSErrorWithoutCode('cannot reuse ports between RPC instances');
        return true;
    })
        .parseSync());
}
 ...
 */
async function setupDevnet(prefundAddress, args) {
 ...
 */
async function inputAccounts(args) {
    const accounts = [];
        ...
        if (!isFile) {
            for (const addressString of addresses) {
                ...
                if (address.equals(derivedAddress) === true) {
                    ...
                    throw EthereumJSErrorWithoutCode(`Private key does not match for ${address} (address derived: ${derivedAddress})`);
                }
            }
        }
        else {
...
export async function getCryptoFunctions(useJsCrypto) {
    ...
    if (useJsCrypto === false) {
        ...
        cryptoFunctions.ecrecover = (msgHash, v, r, s, chainID) => secp256k1Expand(secp256k1Recover(msgHash, concatBytes(setLengthLeft(r, 32), setLengthLeft(s, 32)), Number(calculateSigRecovery(v, chainID)))).slice(1);
        cryptoFunctions.sha256 = wasmSha256;
        cryptoFunctions.ecsign = (msg, pk) => {
...
export async function generateClientConfig(args) {
    ...
    if (args.mine === true && accounts.length === 0) {
        throw EthereumJSErrorWithoutCode('Please provide an account to mine blocks with `--unlock [address]` or use `--dev` to generate');
    }
    ...
    const config = new Config({
        ...
        engineNewpayloadMaxExecute: args.ignoreStatelessInvalidExecs === true || args.skipEngineExec === true
            ? 0
