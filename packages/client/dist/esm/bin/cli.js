...
async function startExecutionFrom(client) {
    if (args.startExecutionFrom === undefined)
        return;
    const startExecutionFrom = BigInt(args.startExecutionFrom);
    const height = client.chain.headers.height;
    if (height < startExecutionFrom) {
        throw EthereumJSErrorWithoutCode(`Cannot start merkle chain higher than current height ${height}`);
    }
    ...
    if (client.config.execCommon.hardforkGteHardfork(startExecutionHardfork, Hardfork.Verkle)) {
        if (client.config.statelessVerkle) {
            ...
            try {
                ...
                client.config.logger?.info(`vmHead set to ${client.chain.headers.height} for starting stateless execution at hardfork=${startExecutionHardfork}`);
            }
            catch (err) {
                throw EthereumJSErrorWithoutCode(`Error setting vmHead for starting stateless execution: ${err}`);
            }
        }
            ...
            try {
                await client.chain.blockchain.setIteratorHead('vm', startExecutionParent.hash());
                await client.chain.update(false);
                client.config.logger?.info(`vmHead set to ${client.chain.headers.height} for starting stateful execution at hardfork=${startExecutionHardfork}`);
            }
            catch (err) {
                throw EthereumJSErrorWithoutCode(`Error setting vmHead for starting stateful execution: ${err}`);
            }
        }
        else {
            // we need parent state availability to set the vmHead to the parent
            throw EthereumJSErrorWithoutCode(`Stateful execution reset not implemented at hardfork=${startExecutionHardfork}`);
        }
    }
}
/**
...
async function run() {
    ...
    const { config, customGenesisState, customGenesisStateRoot, metricsServer } = await generateClientConfig(args);
