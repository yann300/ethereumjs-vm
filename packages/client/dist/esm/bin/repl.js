...
const setupClient = async (config, customGenesisState, common, args) => {
    const client = await createInlineClient(config, common, customGenesisState, args.dataDir ?? '', true);
...
const activateRPCMethods = async (replServer, allRPCMethods) => {
    function defineRPCAction(context, methodName, params) {
        let parsedParams;
    ...
    replServer.defineCommand('logLevel', {
        ...
        action(params) {
            ...
            if (['debug', 'info', 'warn', 'error'].includes(level)) {
                const logger = replServer.context.client.config.logger;
                if (logger === undefined) {
                    this.displayPrompt();
                    return;
                }
                for (const transport of replServer.context.client.config.logger
                    .transports) {
...
const setupRepl = async (args) => {
    ...
    const { client, executionRPC, engineRPC } = await setupClient(config, customGenesisState, // TODO: figure out if this param is mandatory
    common, args);
