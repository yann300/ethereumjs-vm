...
import { EthereumJSErrorWithoutCode, bytesToUnprefixedHex, hexToBytes, randomBytes, } from '@ethereumjs/util';
import { RPCManager, saveReceiptsMethods } from "../src/rpc/index.js";
import * as modules from "../src/rpc/modules/index.js";
import { MethodConfig, createRPCServer, createRPCServerListener, createWsRPCServerListener, } from "../src/util/index.js";
/**
 ...
 */
export function startRPCServers(client, args) {
    const { config } = client;
    const servers = [];
    ...
    if ((rpc || rpcEngine) && !config.saveReceipts) {
        logger?.warn(`Starting client without --saveReceipts might lead to interop issues with a CL especially if the CL intends to propose blocks, omitting methods=${saveReceiptsMethods}`);
    }
    if (rpc || ws) {
        let rpcHttpServer;
...
export function helpRPC() {
    ...
    for (const modName of modules.list) {
        console.log(`${modName}:`);
        const methods = RPCManager.getMethodNames(modules[modName]);
