...
function evp_kdf(data, salt, opts) {
    ...
    function iter(block) {
        if (params.digest !== 'md5')
            throw (0, util_1.EthereumJSErrorWithoutCode)('Only md5 is supported in evp_kdf');
...
async function fromEtherWallet(input, password) {
    ...
    if (!json.locked) {
        if (json.private.length !== 64) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Invalid private key length');
        }
        privateKey = (0, util_1.unprefixedHexToBytes)(json.private);
    }
    else {
        if (typeof password !== 'string') {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Password required');
        }
        if (password.length < 7) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Password must be at least 7 characters');
        }
        ...
        if (!cipher.salt) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Unsupported EtherWallet key format');
        }
        // derive key/iv using OpenSSL EVP as implemented in CryptoJS
        const evp = evp_kdf((0, util_1.utf8ToBytes)(password), cipher.salt, { keysize: 32, ivsize: 16 });
        const pr = (0, aes_js_1.decrypt)(cipher.ciphertext, evp.key, evp.iv, 'aes-256-cbc');
        // NOTE: yes, they've run it through UTF8
        privateKey = (0, util_1.unprefixedHexToBytes)((0, util_1.bytesToUtf8)(pr));
    }
    const wallet = new wallet_ts_1.Wallet(privateKey);
    if (wallet.getAddressString() !== json.address) {
        throw (0, util_1.EthereumJSErrorWithoutCode)('Invalid private key or address');
    }
    return wallet;
}
/**
...
function fromEtherCamp(passphrase) {
    return new wallet_ts_1.Wallet((0, keccak_js_1.keccak256)((0, util_1.utf8ToBytes)(passphrase)));
}
/**
...
function fromQuorumWallet(passphrase, userid) {
    if (passphrase.length < 10) {
        throw (0, util_1.EthereumJSErrorWithoutCode)('Passphrase must be at least 10 characters');
    }
    if (userid.length < 10) {
        throw (0, util_1.EthereumJSErrorWithoutCode)('User id must be at least 10 characters');
    }
    const merged = (0, util_1.utf8ToBytes)(passphrase + userid);
    const seed = (0, pbkdf2_js_1.pbkdf2Sync)(merged, merged, 2000, 32, 'sha256');
    return new wallet_ts_1.Wallet(seed);
}
exports.Thirdparty = {
    fromEtherWallet,
    fromEtherCamp,
