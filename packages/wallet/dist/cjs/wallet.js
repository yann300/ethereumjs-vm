...
exports.Wallet = exports.KDFFunctions = void 0;
// cspell:ignore uuidv kdfparams dklen xprv xpub
const util_1 = require("@ethereumjs/util");
const base_1 = require("@scure/base");
const aes = require("ethereum-cryptography/aes.js");
...
const bs58check = (0, base_1.base58check)(sha256_js_1.sha256);
function scryptV1(password, salt, kdfparams) {
    const { N, P, R, DkLen } = kdfparams;
    return (0, scrypt_js_1.scrypt)(password, salt, N, P, R, DkLen);
}
function scryptV3(password, kdfparams) {
    const { salt, n, p, r, dklen } = kdfparams;
    return (0, scrypt_js_1.scrypt)((0, util_1.utf8ToBytes)(password), salt, n, p, r, dklen);
}
function scryptV3Out(password, kdfparams) {
    const { salt, n, p, r, dklen } = kdfparams;
    return (0, scrypt_js_1.scrypt)((0, util_1.utf8ToBytes)(password), (0, util_1.unprefixedHexToBytes)(salt), n, p, r, dklen);
}
function validateHexString(paramName, str, length) {
    ...
    if (length % 2) {
        throw (0, util_1.EthereumJSErrorWithoutCode)(`Invalid length argument, must be an even number`);
    }
    if (typeof length === 'number' && str.length !== length) {
        throw (0, util_1.EthereumJSErrorWithoutCode)(`Invalid ${paramName}, string must be ${length} hex characters`);
    }
    if (!/^([0-9a-f]{2})+$/i.test(str)) {
        const howMany = typeof length === 'number' ? length : 'empty or a non-zero even number of';
        throw (0, util_1.EthereumJSErrorWithoutCode)(`Invalid ${paramName}, string must be ${howMany} hex characters`);
    }
    return str;
}
function validateBytes(paramName, bytes, length) {
    if (!(bytes instanceof Uint8Array)) {
        ...
        throw (0, util_1.EthereumJSErrorWithoutCode)(`Invalid ${paramName}, must be a string (${howManyHex} hex characters) or Uint8Array${howManyBytes}`);
    }
    if (typeof length === 'number' && bytes.length !== length) {
        throw (0, util_1.EthereumJSErrorWithoutCode)(`Invalid ${paramName}, Uint8Array must be ${length} bytes`);
    }
    return bytes;
}
function mergeToV3ParamsWithDefaults(params) {
    const v3Defaults = {
        cipher: 'aes-128-ctr',
        kdf: 'scrypt',
        salt: (0, util_1.randomBytes)(32),
        iv: (0, util_1.randomBytes)(16),
        uuid: (0, util_1.randomBytes)(16),
        dklen: 32,
        c: 262144,
        n: 262144,
        r: 8,
    ...
    if (typeof params.salt === 'string') {
        params.salt = (0, util_1.unprefixedHexToBytes)(validateHexString('salt', params.salt));
    }
    if (typeof params.iv === 'string') {
        params.iv = (0, util_1.unprefixedHexToBytes)(validateHexString('iv', params.iv, 32));
    }
    if (typeof params.uuid === 'string') {
        params.uuid = (0, util_1.unprefixedHexToBytes)(validateHexString('uuid', params.uuid, 32));
    }
    if (params.salt) {
...
class Wallet {
    constructor(privateKey, publicKey) {
        ...
        if (privateKey && publicKey) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Cannot supply both a private and a public key to the constructor');
        }
        if (privateKey && !(0, util_1.isValidPrivate)(privateKey)) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Private key does not satisfy the curve requirements (ie. it is invalid)');
        }
        if (publicKey && !(0, util_1.isValidPublic)(publicKey)) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Invalid public key');
        }
    }
    // static methods
    /**
    ...
    static fromExtendedPublicKey(extendedPublicKey) {
        if (extendedPublicKey.slice(0, 4) !== 'xpub') {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Not an extended public key');
        }
    ...
    static fromExtendedPrivateKey(extendedPrivateKey) {
        if (extendedPrivateKey.slice(0, 4) !== 'xprv') {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Not an extended private key');
        }
        const tmp = bs58check.decode(extendedPrivateKey);
        if (tmp[45] !== 0) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Invalid extended private key');
        }
    ...
    static async fromV1(input, password) {
        ...
        if (json.Version !== '1') {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Not a V1 Wallet');
        }
        if (json.Crypto.KeyHeader.Kdf !== 'scrypt') {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Unsupported key derivation scheme');
        }
        ...
        if ((0, util_1.bytesToUnprefixedHex)(mac) !== json.Crypto.MAC) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Key derivation failed - possibly wrong passphrase');
        }
        const seed = aes.decrypt(ciphertext, (0, keccak_js_1.keccak256)(derivedKey.subarray(0, 16)).subarray(0, 16), (0, util_1.unprefixedHexToBytes)(json.Crypto.IV), 'aes-128-cbc');
    ...
    static async fromV3(input, password, nonStrict = false) {
        ...
        if (json.version !== 3) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Not a V3 wallet');
        }
        ...
        if (json.crypto.kdf === 'scrypt') {
            ...
            if (kdfparams.prf !== 'hmac-sha256') {
                throw (0, util_1.EthereumJSErrorWithoutCode)('Unsupported parameters to PBKDF2');
            }
        ...
        const seed = aes.decrypt(ciphertext, derivedKey.subarray(0, 16), (0, util_1.unprefixedHexToBytes)(json.crypto.cipherparams.iv), json.crypto.cipher);
        return new Wallet(seed);
    }
    /*
    ...
    static async fromEthSale(input, password) {
        const json = typeof input === 'object' ? input : JSON.parse(input);
        const encseed = (0, util_1.unprefixedHexToBytes)(json.encseed);
        // key derivation
        const pass = (0, util_1.utf8ToBytes)(password);
        const derivedKey = (await (0, pbkdf2_js_1.pbkdf2)(pass, pass, 2000, 32, 'sha256')).subarray(0, 16);
        // seed decoding (IV is first 16 bytes)
        // NOTE: crypto (derived from openssl) when used with aes-*-cbc will handle PKCS#7 padding internally
        //       see also http://stackoverflow.com/a/31614770/4964819
        const seed = aes.decrypt(encseed.subarray(16), derivedKey, encseed.subarray(0, 16), 'aes-128-cbc', true);
        const wallet = new Wallet((0, keccak_js_1.keccak256)(seed));
        if ((0, util_1.bytesToUnprefixedHex)(wallet.getAddress()) !== json.ethaddr) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('Decoded key mismatch - possibly wrong passphrase');
        }
        return wallet;
    }
    // private getters
    ...
    getPrivateKeyString() {
        return (0, util_1.bytesToHex)(this.privKey);
    }
    /**
     * Returns the wallet's public key.
     */
    ...
    async toV3(password, opts) {
        ...
        let derivedKey;
        switch (v3Params.kdf) {
            case exports.KDFFunctions.PBKDF:
                ...
                break;
            case exports.KDFFunctions.Scrypt:
                kdfParams = kdfParamsForScrypt(v3Params);
                // FIXME: support progress reporting callback
                derivedKey = await scryptV3(password, kdfParams);
                break;
            default:
                throw (0, util_1.EthereumJSErrorWithoutCode)('Unsupported kdf');
        }
        const ciphertext = aes.encrypt(this.privKey, derivedKey.subarray(0, 16), v3Params.iv, v3Params.cipher, false);
        const mac = (0, keccak_js_1.keccak256)((0, util_1.concatBytes)(derivedKey.subarray(16, 32), ciphertext));
        return {
            version: 3,
