...
import { EthereumJSErrorWithoutCode, bytesToHex, bytesToUnprefixedHex, concatBytes, equalsBytes, importPublic, isValidPrivate, isValidPublic, privateToAddress, privateToPublic, publicToAddress, randomBytes, toChecksumAddress, unprefixedHexToBytes, utf8ToBytes, } from '@ethereumjs/util';
...
function validateHexString(paramName, str, length) {
    ...
    if (typeof length === 'number' && str.length !== length) {
        throw EthereumJSErrorWithoutCode(`Invalid ${paramName}, string must be ${length} hex characters`);
    }
...
export class Wallet {
    ...
    static async fromV1(input, password) {
        ...
        const seed = aes.decrypt(ciphertext, keccak256(derivedKey.subarray(0, 16)).subarray(0, 16), unprefixedHexToBytes(json.Crypto.IV), 'aes-128-cbc');
    ...
    static async fromV3(input, password, nonStrict = false) {
        ...
        if (json.crypto.kdf === 'scrypt') {
            ...
            derivedKey = await pbkdf2(utf8ToBytes(password), unprefixedHexToBytes(kdfparams.salt), kdfparams.c, kdfparams.dklen, 'sha256');
        }
        else {
            throw EthereumJSErrorWithoutCode('Unsupported key derivation scheme');
        }
        ...
        const seed = aes.decrypt(ciphertext, derivedKey.subarray(0, 16), unprefixedHexToBytes(json.crypto.cipherparams.iv), json.crypto.cipher);
        return new Wallet(seed);
    }
    /*
    ...
    async toV3(password, opts) {
        ...
        switch (v3Params.kdf) {
            case KDFFunctions.PBKDF:
                ...
                derivedKey = await pbkdf2(utf8ToBytes(password), kdfParams.salt, kdfParams.c, kdfParams.dklen, 'sha256');
                break;
