...
class Miner {
    ...
    constructor(mineObject, ethash) {
        if (mineObject instanceof block_1.BlockHeader) {
            ...
            throw (0, util_1.EthereumJSErrorWithoutCode)('unsupported mineObject');
        }
        this.currentNonce = util_1.BIGINT_0;
        this.ethash = ethash;
    ...
    async mine(iterations = 0) {
        ...
        if (solution) {
            if (this.block) {
                ...
                return (0, block_1.createBlock)(data, { common: this.block.common });
            }
            else {
                const data = this.blockHeader.toJSON();
                data.mixHash = solution.mixHash;
                data.nonce = solution.nonce;
                return (0, block_1.createBlockHeader)(data, { common: this.blockHeader.common });
            }
        }
    }
    /**
    ...
    async iterate(iterations = 0) {
        ...
        while (iterations !== 0 && !this.stopMining) {
            ...
            const solution = await new Promise((resolve) => {
                setTimeout(() => {
                    const nonce = (0, util_1.setLengthLeft)((0, util_1.bigIntToBytes)(this.currentNonce), 8);
                    const a = this.ethash.run(headerHash, nonce);
                    const result = (0, util_1.bytesToBigInt)(a.hash);
...
class Ethash {
    ...
    mkcache(cacheSize, seed) {
        const n = Math.floor(cacheSize / util_ts_1.params.HASH_BYTES);
        const o = [(0, keccak_js_1.keccak512)(seed)];
        let i;
        for (i = 1; i < n; i++) {
            o.push((0, keccak_js_1.keccak512)(o[o.length - 1]));
        }
        for (let _ = 0; _ < util_ts_1.params.CACHE_ROUNDS; _++) {
            for (i = 0; i < n; i++) {
                const v = new DataView(o[i].buffer).getUint32(0, true) % n;
                o[i] = (0, keccak_js_1.keccak512)(xor(o[(i - 1 + n) % n], o[v]));
            }
        }
        this.cache = o;
        return this.cache;
    ...
    calcDatasetItem(i) {
        ...
        const r = Math.floor(util_ts_1.params.HASH_BYTES / util_ts_1.params.WORD_BYTES);
        ...
        mix = (0, keccak_js_1.keccak512)(mix);
        for (let j = 0; j < util_ts_1.params.DATASET_PARENTS; j++) {
            const cacheIndex = (0, util_ts_1.fnv)(i ^ j, new DataView(mix.buffer).getUint32((j % r) * 4, true));
            mix = (0, util_ts_1.fnvBytes)(mix, this.cache[cacheIndex % n]);
        }
        return (0, keccak_js_1.keccak512)(mix);
    }
    run(val, nonce, fullSize) {
        if (fullSize === undefined) {
            if (this.fullSize === undefined) {
                throw (0, util_1.EthereumJSErrorWithoutCode)('fullSize needed');
            }
            else {
                fullSize = this.fullSize;
            }
        }
        const n = Math.floor(fullSize / util_ts_1.params.HASH_BYTES);
        const w = Math.floor(util_ts_1.params.MIX_BYTES / util_ts_1.params.WORD_BYTES);
        const s = (0, keccak_js_1.keccak512)((0, util_1.concatBytes)(val, (0, util_ts_1.bytesReverse)(nonce)));
        const mixhashes = Math.floor(util_ts_1.params.MIX_BYTES / util_ts_1.params.HASH_BYTES);
        let mix = (0, util_1.concatBytes)(...Array(mixhashes).fill(s));
        let i;
        for (i = 0; i < util_ts_1.params.ACCESSES; i++) {
            const p = ((0, util_ts_1.fnv)(i ^ new DataView(s.buffer).getUint32(0, true), new DataView(mix.buffer).getUint32((i % w) * 4, true)) %
            ...
            mix = (0, util_ts_1.fnvBytes)(mix, (0, util_1.concatBytes)(...newdata));
        }
        const cmix = new Uint8Array(mix.length / 4);
        const cmixView = new DataView(cmix.buffer);
        const mixView = new DataView(mix.buffer);
        for (i = 0; i < mix.length / 4; i = i + 4) {
            const a = (0, util_ts_1.fnv)(mixView.getUint32(i * 4, true), mixView.getUint32((i + 1) * 4, true));
            const b = (0, util_ts_1.fnv)(a, mixView.getUint32((i + 2) * 4, true));
            const c = (0, util_ts_1.fnv)(b, mixView.getUint32((i + 3) * 4, true));
            cmixView.setUint32(i, c, true);
        }
        return {
            mix: cmix,
            hash: (0, keccak_js_1.keccak256)((0, util_1.concatBytes)(s, cmix)),
        };
    }
    cacheHash() {
        ...
        return (0, keccak_js_1.keccak256)(result);
    }
    headerHash(rawHeader) {
        return (0, keccak_js_1.keccak256)(rlp_1.RLP.encode(rawHeader.slice(0, -2)));
    }
    /**
     * Loads the seed and cache given a block number.
     */
    async loadEpoc(number) {
        const epoc = (0, util_ts_1.getEpoc)(number);
        if (this.epoc === epoc) {
            return;
        }
        this.epoc = epoc;
        if (!this.cacheDB) {
            throw (0, util_1.EthereumJSErrorWithoutCode)('cacheDB needed');
        }
        ...
        const findLastSeed = async (epoc) => {
            ...
            const dbData = await this.cacheDB.get(epoc, {
                keyEncoding: util_1.KeyEncoding.Number,
                valueEncoding: util_1.ValueEncoding.JSON,
            });
            if (dbData !== undefined) {
                const data = {
                    cache: dbData.cache.map((el) => (0, util_1.hexToBytes)(el)),
                    fullSize: dbData.fullSize,
                    cacheSize: dbData.cacheSize,
                    seed: (0, util_1.hexToBytes)(dbData.seed),
                };
        ...
        if (!data) {
            this.cacheSize = await (0, util_ts_1.getCacheSize)(epoc);
            this.fullSize = await (0, util_ts_1.getFullSize)(epoc);
            const [seed, foundEpoc] = await findLastSeed(epoc);
            this.seed = (0, util_ts_1.getSeed)(seed, foundEpoc, epoc);
            const cache = this.mkcache(this.cacheSize, this.seed);
            // store the generated cache
            await this.cacheDB.put(epoc, {
                cacheSize: this.cacheSize,
                fullSize: this.fullSize,
                seed: (0, util_1.bytesToHex)(this.seed),
                cache: cache.map((el) => (0, util_1.bytesToHex)(el)),
            }, {
                keyEncoding: util_1.KeyEncoding.Number,
                valueEncoding: util_1.ValueEncoding.JSON,
            });
    ...
    async _verifyPOW(header) {
        ...
        const result = (0, util_1.bytesToBigInt)(a.hash);
        return (0, util_1.equalsBytes)(a.mix, mixHash) && util_1.TWO_POW256 / difficulty > result;
    }
