...
async function _runTx(vm, opts) {
    ...
    if (vm.common.isActivatedEIP(6800)) {
        ...
        if (!('verifyVerklePostState' in vm.stateManager)) {
            throw (0, util_1.EthereumJSErrorWithoutCode)(`Verkle State Manager needed for execution of verkle blocks`);
        }
        stateAccesses = vm.evm.verkleAccessWitness;
        txAccesses = new evm_1.VerkleAccessWitness({
    ...
    if (vm.common.isActivatedEIP(6800) && results.minerValue !== util_1.BIGINT_0) {
        if (vm.evm.verkleAccessWitness === undefined) {
            throw Error(`verkleAccessWitness required if verkle (EIP-6800) is activated`);
        }
