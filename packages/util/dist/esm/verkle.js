 ...
 */
export function getVerkleStem(verkleCrypto, address, treeIndex = 0) {
 ...
 */
export function verifyVerkleProof(verkleCrypto, executionWitness) {
 ...
 */
export function getVerkleTreeIndicesForStorageSlot(storageKey) {
    let position;
 ...
 */
export function decodeVerkleLeafBasicData(encodedBasicData) {
    const versionBytes = encodedBasicData.slice(0, VERKLE_VERSION_BYTES_LENGTH);
    const nonceBytes = encodedBasicData.slice(VERKLE_NONCE_OFFSET, VERKLE_NONCE_OFFSET + VERKLE_NONCE_BYTES_LENGTH);
    const codeSizeBytes = encodedBasicData.slice(VERKLE_CODE_SIZE_OFFSET, VERKLE_CODE_SIZE_OFFSET + VERKLE_CODE_SIZE_BYTES_LENGTH);
    const balanceBytes = encodedBasicData.slice(VERKLE_BALANCE_OFFSET, VERKLE_BALANCE_OFFSET + VERKLE_BALANCE_BYTES_LENGTH);
 ...
 */
export function encodeVerkleLeafBasicData(account) {
    ...
    const encodedCodeSize = setLengthLeft(int32ToBytes(account.codeSize), VERKLE_CODE_SIZE_BYTES_LENGTH);
 ...
 */
export const generateChunkSuffixes = (numChunks) => {
    if (numChunks === 0)
        return [];
 ...
 */
export const generateCodeStems = async (numChunks, address, verkleCrypto) => {
    ...
    for (let stemNum = 0; stemNum < numStems - 1; stemNum++) {
        ...
        const firstChunkKey = await getVerkleTreeKeyForCodeChunk(address, VERKLE_CODE_OFFSET + stemNum * VERKLE_NODE_WIDTH, verkleCrypto);
