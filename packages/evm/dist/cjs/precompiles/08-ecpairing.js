...
function precompile08(opts) {
    ...
    if (opts._debug !== undefined) {
        opts._debug(`Run ECPAIRING (0x08) precompile data=${(0, util_1.short)(opts.data)} length=${opts.data.length} gasLimit=${opts.gasLimit} gasUsed=${gasUsed}`);
    }
    ...
    const returnData = (0, util_1.hexToBytes)(opts._EVM['_bn128'].ec_pairing((0, util_1.bytesToUnprefixedHex)(inputData)));
    ...
    if (opts._debug !== undefined) {
        opts._debug(`ECPAIRING (0x08) return value=${(0, util_1.bytesToHex)(returnData)}`);
    }
    return {
