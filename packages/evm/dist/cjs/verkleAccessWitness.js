...
exports.generateExecutionWitness = exports.VerkleAccessWitness = void 0;
...
const debug = (0, debug_1.default)('evm:verkle:aw');
/**
 * Tree key constants.
 */
...
function decodeAccessedState(treeIndex, chunkIndex) {
    ...
    switch (position) {
        case BigInt(0):
            return { type: common_1.VerkleAccessedStateType.BasicData };
        case BigInt(1):
            return { type: common_1.VerkleAccessedStateType.CodeHash };
        default:
            if (position < util_1.VERKLE_HEADER_STORAGE_OFFSET) {
                throw Error(`No attribute yet stored >=2 and <${util_1.VERKLE_HEADER_STORAGE_OFFSET}`);
            }
            if (position >= util_1.VERKLE_HEADER_STORAGE_OFFSET && position < util_1.VERKLE_CODE_OFFSET) {
                ...
                return { type: common_1.VerkleAccessedStateType.Storage, slot };
            }
                ...
                return {
                    type: common_1.VerkleAccessedStateType.Code,
...
class VerkleAccessWitness {
    ...
    readAccountBasicData(address) {
        return this.touchAddressOnReadAndComputeGas(address, 0, util_1.VERKLE_BASIC_DATA_LEAF_KEY);
    }
    writeAccountBasicData(address) {
        return this.touchAddressOnWriteAndComputeGas(address, 0, util_1.VERKLE_BASIC_DATA_LEAF_KEY);
    }
    readAccountCodeHash(address) {
        return this.touchAddressOnReadAndComputeGas(address, 0, util_1.VERKLE_CODE_HASH_LEAF_KEY);
    }
    writeAccountCodeHash(address) {
        return this.touchAddressOnWriteAndComputeGas(address, 0, util_1.VERKLE_CODE_HASH_LEAF_KEY);
    }
    ...
    readAccountStorage(address, storageSlot) {
        const { treeIndex, subIndex } = (0, util_1.getVerkleTreeIndicesForStorageSlot)(storageSlot);
 ...
 */
const generateExecutionWitness = async (stateManager, accessWitness, parentStateRoot) => {
    const trie = stateManager['_trie'];
    ...
    for (const stem of accessedSuffixes.keys()) {
        ...
        if (suffixes === undefined || suffixes.length === 0)
            continue;
        const currentValues = await trie.get((0, util_1.hexToBytes)(stem), suffixes);
        trie.root(postStateRoot);
        const newValues = await trie.get((0, util_1.hexToBytes)(stem), suffixes);
        const stemStateDiff = [];
...
exports.generateExecutionWitness = generateExecutionWitness;
