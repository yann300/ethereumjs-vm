...
class Journal {
    constructor(stateManager, common) {
        ...
        this._debug = (0, debug_1.default)('evm:journal');
    ...
    touchAccount(address) {
        ...
        if (this.preimages !== undefined) {
            const bytesAddress = (0, util_1.unprefixedHexToBytes)(address);
            if (this.stateManager.getAppliedKey === undefined) {
                throw (0, util_1.EthereumJSErrorWithoutCode)('touchAccount: stateManager.getAppliedKey can not be undefined if preimage storing is enabled');
            }
            const hashedKey = this.stateManager.getAppliedKey(bytesAddress);
            this.preimages.set((0, util_1.bytesToHex)(hashedKey), bytesAddress);
        }
    ...
    async revert() {
        ...
        for (let i = this.journalDiff.length - 1; i >= 0; i--) {
            ...
            for (const address of touchedSet) {
                ...
                if (address !== util_1.RIPEMD160_ADDRESS_STRING) {
    ...
    async cleanup() {
        if (this.common.gteHardfork(common_1.Hardfork.SpuriousDragon)) {
            for (const addressHex of this.touched) {
                const address = new util_1.Address((0, util_1.hexToBytes)(`0x${addressHex}`));
    ...
    addAlwaysWarmAddress(addressStr, addToAccessList = false) {
        const address = (0, util_1.stripHexPrefix)(addressStr);
    ...
    addAlwaysWarmSlot(addressStr, slotStr, addToAccessList = false) {
        ...
        const slot = (0, util_1.stripHexPrefix)(slotStr);
        slotsSet.add(slot);
        if (addToAccessList && this.accessList !== undefined) {
    ...
    isWarmedAddress(address) {
        const addressHex = (0, util_1.bytesToUnprefixedHex)(address);
    ...
    addWarmedAddress(addressArr) {
        const address = (0, util_1.bytesToUnprefixedHex)(addressArr);
        if (!this.journal.has(address)) {
    ...
    isWarmedStorage(address, slot) {
        ...
        if (slots === undefined) {
            if (this.alwaysWarmJournal.has(addressHex)) {
                return this.alwaysWarmJournal.get(addressHex).has((0, util_1.bytesToUnprefixedHex)(slot));
            }
            return false;
        }
        if (slots.has((0, util_1.bytesToUnprefixedHex)(slot))) {
            return true;
        }
    ...
    addWarmedStorage(address, slot) {
        ...
        const slotStr = (0, util_1.bytesToUnprefixedHex)(slot);
