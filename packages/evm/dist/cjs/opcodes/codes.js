...
const functions_ts_1 = require("./functions.js");
const gas_ts_1 = require("./gas.js");
...
const hardforkOpcodes = [
    ...
    {
        hardfork: common_1.Hardfork.Paris,
        opcodes: {
...
function createOpcodes(opcodes) {
    ...
    for (const [key, value] of Object.entries(opcodes)) {
        ...
        result.set(code, new Opcode({
            code,
            fullName: (0, util_ts_1.getFullname)(code, value.name),
            ...value,
        }));
    }
    return result;
...
function getOpcodesForHF(common, customOpcodes) {
    ...
    const handlersCopy = new Map(functions_ts_1.handlers);
    const dynamicGasHandlersCopy = new Map(gas_ts_1.dynamicGasHandlers);
    ...
    for (const key in opcodeBuilder) {
        ...
        if (baseFee === undefined) {
            throw (0, util_1.EthereumJSErrorWithoutCode)(`base fee not defined for: ${opcodeBuilder[key].name}`);
        }
    ...
    if (customOpcodes) {
        for (const _code of customOpcodes) {
            const code = _code;
            if (!(0, types_ts_1.isAddOpcode)(code)) {
