...
exports.ContainerSectionType = void 0;
exports.verifyCode = verifyCode;
const errors_ts_1 = require("./errors.js");
const stackDelta_ts_1 = require("./stackDelta.js");
exports.ContainerSectionType = {
 ...
 */
function verifyCode(container, evm, mode = exports.ContainerSectionType.RuntimeCode) {
...
function validateOpcodes(container, evm, mode = exports.ContainerSectionType.RuntimeCode) {
    ...
    for (const code of container.body.codeSections) {
        ...
        function addJump(location) {
            if (intermediateBytes.has(location)) {
                ...
                (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_RJUMP);
            }
        ...
        while (ptr < code.length) {
            ...
            if (!reachableOpcodes.has(ptr)) {
                (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.UNREACHABLE_CODE);
            }
            ...
            const opcodeInputs = stackDelta_ts_1.stackDelta[opcode].inputs;
            const opcodeOutputs = stackDelta_ts_1.stackDelta[opcode].outputs;
            if (minStackCurrent - opcodeInputs < 0) {
                (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.STACK_UNDERFLOW);
            }
            ...
            if (maxStackNext > 1024) {
                ...
                (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.STACK_OVERFLOW);
            }
            if (nonReturningFunction && opcode === 0xe4) {
                (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_RETURNING_SECTION);
            }
            lastOpcode = opcode;
            if (!opcodeNumbers.has(opcode)) {
                (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_OPCODE);
            }
            if (opcode === 0xe0 || opcode === 0xe1) {
                ...
                if (tableSize === undefined) {
                    (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.OPCODE_INTERMEDIATES_OOB);
                }
                else if (tableSize === 0) {
                    (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.RJUMPV_TABLE_SIZE0);
                }
                ...
                if (target >= container.header.codeSizes.length) {
                    (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_CALL_TARGET);
                }
                if (opcode === 0xe3) {
                    // CALLF
                    ...
                    if (targetOutputs === 0x80) {
                        ...
                        (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_CALLF_RETURNING);
                    }
                    ...
                    if (targetOutputs > currentOutputs && !targetNonReturning) {
                        ...
                        (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_JUMPF);
                    }
                    ...
                    if (targetNonReturning) {
                        ...
                        if (!(minStackCurrent === maxStackCurrent && maxStackCurrent === expectedStack)) {
                            (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_STACK_HEIGHT);
                        }
                ...
                if (target >= container.header.containerSizes.length) {
                    (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_EOF_CREATE_TARGET);
                }
                if (containerTypeMap.has(target)) {
                    if (containerTypeMap.get(target) !== exports.ContainerSectionType.InitCode) {
                        (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.CONTAINER_DOUBLE_TYPE);
                    }
                }
                containerTypeMap.set(target, exports.ContainerSectionType.InitCode);
            }
            else if (opcode === 0xee) {
                // RETURNCONTRACT
                if (mode !== exports.ContainerSectionType.InitCode) {
                    (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.CONTAINER_TYPE_ERROR);
                }
                const target = code[ptr + 1];
                if (target >= container.header.containerSizes.length) {
                    (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_RETURN_CONTRACT_TARGET);
                }
                if (containerTypeMap.has(target)) {
                    if (containerTypeMap.get(target) !== exports.ContainerSectionType.DeploymentCode) {
                        (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.CONTAINER_DOUBLE_TYPE);
                    }
                }
                containerTypeMap.set(target, exports.ContainerSectionType.DeploymentCode);
            }
            else if (opcode === 0xd1) {
                // DATALOADN
                ...
                if (container.header.dataSize < endOfSlice) {
                    (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.DATALOADN_OOB);
                }
            }
            ...
            const intermediates = stackDelta_ts_1.stackDelta[opcode].intermediates;
            if (intermediates > 0) {
            ...
            if (stackDelta_ts_1.stackDelta[opcode].terminating === undefined) {
            ...
            for (const successor of successorSet) {
                if (successor < ptr) {
                    ...
                    if (stackHeightMin[successor] !== minStackNext ||
                        ...
                        (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.UNSTABLE_STACK);
                    }
                }
        ...
        if (!terminatingOpcodes.has(lastOpcode)) {
            (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.INVALID_TERMINATOR);
        }
        if (container.body.typeSections[codeSection].maxStackHeight !== maxStackHeight) {
            (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.MAX_STACK_HEIGHT_VIOLATION);
        }
        if (maxStackHeight > 1024) {
            // TODO verify if 1023 or 1024 is the right constant
            (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.MAX_STACK_HEIGHT_LIMIT);
        }
        // Validate that if the section is returning, there is a returning opcode
        if (!sectionHasReturningOpcode && !nonReturningFunction) {
            (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.RETURNING_NO_RETURN);
        }
    }
    ...
    if (sectionAccumulator.size !== container.header.codeSizes.length) {
        (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.UNREACHABLE_CODE_SECTIONS);
    }
    if (containerTypeMap.size !== container.header.containerSizes.length) {
        (0, errors_ts_1.validationError)(errors_ts_1.EOFErrorMessage.UNREACHABLE_CONTAINER_SECTIONS);
    }
