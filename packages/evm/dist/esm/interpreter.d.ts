...
import { type EVMPerformanceLogger } from './logger.ts';
import { Memory } from './memory.ts';
import { Message } from './message.ts';
import { Stack } from './stack.ts';
import type { BinaryTreeAccessWitnessInterface, Common, StateManagerInterface, VerkleAccessWitnessInterface } from '@ethereumjs/common';
import type { Address, PrefixedHexString } from '@ethereumjs/util';
import type { EVM } from './evm.ts';
import type { Journal } from './journal.ts';
import type { Opcode, OpcodeMapEntry } from './opcodes/index.ts';
import type { Block, EOFEnv, EVMMockBlockchainInterface, EVMProfilerOpts, Log } from './types.ts';
...
export declare class Interpreter {
    ...
    constructor(evm: EVM, stateManager: StateManagerInterface, blockchain: EVMMockBlockchainInterface, env: Env, gasLeft: bigint, journal: Journal, performanceLogs: EVMPerformanceLogger, profilerOpts?: EVMProfilerOpts);
    ...
    _runStepHook(dynamicFee: bigint, gasLeft: bigint, memorySize: bigint): Promise<void>;
    _getValidJumpDestinations(code: Uint8Array): {
        jumps: Uint8Array<ArrayBuffer>;
        pushes: {
            [pc: number]: bigint;
        };
        opcodesCached: any[];
     ...
     */
    call(gasLimit: bigint, address: Address, value: bigint, data: Uint8Array): Promise<bigint>;
    /**
     * Message-call into this account with an alternative account's code.
     */
    callCode(gasLimit: bigint, address: Address, value: bigint, data: Uint8Array): Promise<bigint>;
    /**
     * Sends a message with arbitrary data to a given address path, but disallow
     * state modifications. This includes log, create, selfdestruct and call with
     * a non-zero value.
     */
    callStatic(gasLimit: bigint, address: Address, value: bigint, data: Uint8Array): Promise<bigint>;
    /**
     * Message-call into this account with an alternative account's code, but
     * persisting the current values for sender and value.
     */
    callDelegate(gasLimit: bigint, address: Address, value: bigint, data: Uint8Array): Promise<bigint>;
