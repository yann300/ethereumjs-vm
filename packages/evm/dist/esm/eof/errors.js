...
export function validationError(type, ...args) {
    switch (type) {
        ...
        case EOFErrorMessage.VERIFY_BYTES: {
            ...
            throw EthereumJSErrorWithoutCode(EOFErrorMessage.VERIFY_BYTES + ` at pos: ${args[0]}: ${args[1]}`);
        }
        case EOFErrorMessage.VERIFY_UINT: {
            const pos = args[0];
            ...
            throw EthereumJSErrorWithoutCode(EOFErrorMessage.VERIFY_UINT + `at pos: ${args[0]}: ${args[1]}`);
        }
        case EOFErrorMessage.TYPE_SIZE: {
            throw EthereumJSErrorWithoutCode(EOFErrorMessage.TYPE_SIZE + args[0]);
        }
        case EOFErrorMessage.TYPE_SECTIONS: {
            throw EthereumJSErrorWithoutCode(`${EOFErrorMessage.TYPE_SECTIONS} (types ${args[0]} code ${args[1]})`);
        }
        case EOFErrorMessage.INVALID_TYPE_SIZE: {
        ...
        case EOFErrorMessage.MAX_INPUTS: {
            throw EthereumJSErrorWithoutCode(EOFErrorMessage.MAX_INPUTS + `${args[1]} - code section ${args[0]}`);
        }
        case EOFErrorMessage.MAX_OUTPUTS: {
            throw EthereumJSErrorWithoutCode(EOFErrorMessage.MAX_OUTPUTS + `${args[1]} - code section ${args[0]}`);
        }
        case EOFErrorMessage.CODE_SECTION: {
        ...
        case EOFErrorMessage.MAX_STACK_HEIGHT: {
            throw EthereumJSErrorWithoutCode(`${EOFErrorMessage.MAX_STACK_HEIGHT} - typeSection ${args[0]}: `);
        }
        case EOFErrorMessage.MAX_STACK_HEIGHT_LIMIT: {
            throw EthereumJSErrorWithoutCode(`${EOFErrorMessage.MAX_STACK_HEIGHT_LIMIT}, got: ${args[1]} - typeSection ${args[0]}`);
        }
        case EOFErrorMessage.DANGLING_BYTES: {
