...
import { EthereumJSErrorWithoutCode, bigIntToHex, blobsToCommitments, blobsToProofs, bytesToBigInt, bytesToHex, commitmentsToVersionedHashes, computeVersionedHash, equalsBytes, getBlobs, validateNoLeadingZeroes, } from '@ethereumjs/util';
import { paramsTx } from "../params.js";
import { TransactionType } from "../types.js";
import { accessListBytesToJSON } from "../util/access.js";
import { Blob4844Tx } from "./tx.js";
import { txTypeBytes, validateNotArray } from "../util/internal.js";
const validateBlobTransactionNetworkWrapper = (blobVersionedHashes, blobs, commitments, kzgProofs, version, kzg) => {
    if (!(blobVersionedHashes.length === blobs.length && blobs.length === commitments.length)) {
        throw EthereumJSErrorWithoutCode('Number of blobVersionedHashes, blobs, and commitments not all equal');
    }
    ...
    for (let x = 0; x < blobVersionedHashes.length; x++) {
        ...
        if (computedVersionedHash !== blobVersionedHashes[x]) {
            throw EthereumJSErrorWithoutCode(`commitment for blob at index ${x} does not match versionedHash`);
        }
    }
};
/**
 ...
 */
export function createBlob4844Tx(txData, opts) {
 ...
 */
export function createBlob4844TxFromBytesArray(values, opts = {}) {
    if (opts.common?.customCrypto?.kzg === undefined) {
        throw EthereumJSErrorWithoutCode('A common object with customCrypto.kzg initialized required to instantiate a 4844 blob tx');
    }
    if (values.length !== 11 && values.length !== 14) {
        throw EthereumJSErrorWithoutCode('Invalid EIP-4844 transaction. Only expecting 11 values (for unsigned tx) or 14 values (for signed tx).');
    }
 ...
 */
export function createBlob4844TxFromRLP(serialized, opts = {}) {
    if (opts.common?.customCrypto?.kzg === undefined) {
        throw EthereumJSErrorWithoutCode('A common object with customCrypto.kzg initialized required to instantiate a 4844 blob tx');
    }
    if (equalsBytes(serialized.subarray(0, 1), txTypeBytes(TransactionType.BlobEIP4844)) === false) {
        throw EthereumJSErrorWithoutCode(`Invalid serialized tx input: not an EIP-4844 transaction (wrong tx type, expected: ${TransactionType.BlobEIP4844}, received: ${bytesToHex(serialized.subarray(0, 1))}`);
    }
 ...
 */
export function createBlob4844TxFromSerializedNetworkWrapper(serialized, opts) {
    if (!opts || !opts.common) {
 ...
 */
export function createMinimal4844TxFromNetworkWrapper(txData, opts) {
    if (opts?.common?.customCrypto?.kzg === undefined) {
        throw EthereumJSErrorWithoutCode('A common object with customCrypto.kzg initialized required to instantiate a 4844 blob tx');
    }
 ...
 */
export function blobTxNetworkWrapperToJSON(serialized, opts) {
